...
# (コーテーションを含むオプションは指定不可(例:"-d \"dir1 dir2\""))
#
# [改変] 2026-06-06: 環境変数 TTSP_MAKE_OPT で上書き可能にした．
# 例) TTSP_MAKE_OPT="ENABLE_GCOV=true" でGCOV計装ビルド（fmp3側の
#     Makefile.target が解釈．カバレッジ計測用）．
#
...
simulation()
{
	...
	then
		...
		cp -pf $TTSP_DIR/library/FMP/target/zybo_z7_gcc/*.tcl .
		...
		cp -p fmp fmp.elf
		cmd.exe /C c:\\Xilinx\\SDK\\2018.2\\bin\\xsct.bat -interactive ./jtag.tcl ./ps7_init.tcl ./fmp.elf 
