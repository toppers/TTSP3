...
#  の責任を負わない．
#
#  $Id: ttsp_target.sh 42 2019-03-19 02:58:02Z fujisft-shigihara $
#

#
# 実機(i.MX8mm EVKボード) or QEMU
#l
#USE_QEMU=true
USE_QEMU=false
...
#
CONFIG_OPT="-w -S syslog.o -S banner.o -S serial.o -S logtask.o -S imx8uart.o -o -Wno-unused-but-set-variable -o -D_RUN_TTSP3_"
if [ "$USE_QEMU" == "true" ]
then
...
#
KERNEL_COBJS_TARGET="objs/target_kernel_impl.o objs/chip_kernel_impl.o objs/chip_serial.o objs/core_kernel_impl.o objs/core_timer.o objs/arm64.o objs/gic_kernel_impl.o"
APPL_COBJS_TARGET="objs/ttsp_target_test.o"
# objs/out.o objs/ttsp_test_lib.o objs/log_output.o objs/vasyslog.o objs/t_perror.o objs/strerror.o"
...
simulation()
{
	...
	if [ "$USE_QEMU" == "true" ]
	then
		qemu-system-arm -M xilinx-zynq-a9 -semihosting -m 512M -serial null -serial mon:stdio -nographic -smp 2 -kernel fmp | tee $LOGFILE
	else
		TEMPFILE=`pwd`"/temp"
		...
		cp -p fmp fmp.elf
		cmd.exe /C c:\\Xilinx\\SDK\\2017.2\\bin\\xsct.bat -interactive ./jtag.tcl ./ps7_init.tcl ./fmp.elf 
