...
ttsp_target_stop_tick(void)
{
	...
	/* TIM5 の割り込みを無効化 */
	__HAL_TIM_DISABLE_IT(&Tim5Handle, TIM_IT_UPDATE);
	dis_int(INTNO_TIMER);
}
...
{
	/* 1us 進める */
	// const HRTCNT future = target_hrt_get_current() + 1;
	/* すでに割り込み通知が入っている可能性があるためクリア */
	// __HAL_TIM_CLEAR_FLAG(&Tim5Handle, TIM_SR_UIF);
	/* TIM5 のカウンタを初期化 */
	// __HAL_TIM_SET_COUNTER(&Tim5Handle, 0);
	/* TIM2 を有効化，TIM5 の割り込みを有効化 */
	// ttsp_target_start_tick();
	/* TIM5 を再開 */
	// HAL_TIM_Base_Start_IT(&Tim5Handle);
	// while (target_hrt_get_current() < future) ;
	/* TIM2 を停止，TIM5 の割り込みを無効化 */
	// ttsp_target_stop_tick();

	const uint32_t interval = __HAL_TIM_GET_AUTORELOAD(&Tim5Handle);
	__HAL_TIM_SET_COUNTER(&Tim2Handle, __HAL_TIM_GET_COUNTER(&Tim2Handle) + 1);
	if (interval <= 1) {
		__HAL_TIM_SET_AUTORELOAD(&Tim5Handle, 0xFFFFFFFF);
		ena_int(INTNO_TIMER);
		ras_int(INTNO_TIMER);
		dis_int(INTNO_TIMER);
	} else {
		__HAL_TIM_SET_AUTORELOAD(&Tim5Handle, interval - 1);
	}
	
	// __HAL_TIM_SET_COUNTER(&Tim2Handle, __HAL_TIM_GET_COUNTER(&Tim5Handle) + 1);
	// if (__HAL_TIM_GET_COUNTER(&Tim5Handle) == __HAL_TIM_GET_AUTORELOAD(&Tim5Handle)) {
		// ras_int(INTNO_TIMER);
		// ena_int(INTNO_TIMER);
		// dis_int(INTNO_TIMER);
