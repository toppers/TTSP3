...
/*
 *		ターゲット依存のテストサポート関数（EK-RA8M2 / Cortex-M85 / ASP3 用）
 *
 *  [改変] 2026-06-21: ASP プロファイル ek_ra8m2_gcc 版（M-profile / Cortex-M85 実機）．
 *  ASP mps2_an505_gcc 版（ARMv8-M / Cortex-M33 / QEMU）から複製した．本体ロジック
 *  （NVIC raise_int / udf による UsageFault / target_hrt_* による tick 制御 /
 *  USE_TSKINICTXB アクセサ）は arm_m 系で共通のためそのまま流用できる（mps2 は
 *  SysTick イベント駆動 HRT，ek は GPT0 イベント駆動 HRT だが target_hrt_*
 *  インターフェースは同一）．TTSP3 ライセンス条件(2)に基づく改変明記．
 *  --- 以下 M-profile 共通の記述（元は zybo_z7_gcc A-profile 版から作成）---
