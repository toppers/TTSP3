...
#
CONFIG_OPT="-o -Wno-unused-but-set-variable"
if [ "$USE_QEMU" == "true" ]
then
...
# (コーテーションを含むオプションは指定不可(例:"-d \"dir1 dir2\""))
#
# [改変] 2026-06-08: 環境変数 TTSP_MAKE_OPT で上書き可能にした．
# 例) TTSP_MAKE_OPT="ENABLE_GCOV=true" でGCOV計装ビルド（asp3側の
# Makefile.target/zybo_z7.ld/target_kernel_impl.c の対応版が必要）
...
simulation()
{
	...
	if [ "$USE_QEMU" == "true" ]
	then
		qemu-system-arm -M xilinx-zynq-a9 -semihosting -m 512M -serial null -serial mon:stdio -nographic -kernel asp | tee $LOGFILE
	else
		TEMPFILE=`pwd`"/temp"
		...
		cp -p asp asp.elf
		cmd.exe /C c:\\Xilinx\\SDK\\2019.1\\bin\\xsct.bat -interactive ./jtag.tcl ./ps7_init.tcl ./asp.elf 
