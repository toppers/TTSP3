...
#  の責任を負わない．
# 
#  $Id: ttsp_target.sh 85 2020-07-15 01:22:41Z nces-mtakada $
# 
...
if [ "$USE_QEMU" == "true" ]
then
	CONFIG_OPT="$CONFIG_OPT -o -DTOPPERS_USE_QEMU -o -DHRT_CONFIG1"
fi
...
#  （ASP/FMP/HRMPと同根）．
KERNEL_COBJS_TARGET="objs/chip_kernel_impl.o objs/core_kernel_impl.o objs/gic_kernel_impl.o objs/pl310.o objs/target_kernel_impl.o objs/mpcore_kernel_impl.o objs/target_timer.o objs/sim_timer.o"
...
# (コーテーションを含むオプションは指定不可(例:"-d \"dir1 dir2\""))
#
# 環境変数 TTSP_MAKE_OPT で上書き可能にした（ASP/FMP/HRMP と同様）．
# 例) TTSP_MAKE_OPT="ENABLE_GCOV=true" でGCOV計装ビルド（hrp3側の
#     target/zybo_z7_gcc の GCOV 対応を有効化）．
...
simulation()
{
	...
	if [ "$USE_QEMU" == "true" ]
	then
		qemu-system-arm -M xilinx-zynq-a9 -semihosting -m 512M -serial null -serial mon:stdio -nographic -kernel hrp | tee $LOGFILE
	else
		TEMPFILE=`pwd`"/temp"

		cp -pf $TTSP_DIR/library/ASP/target/zybo_z7_gcc/*.tcl .
		...
		cp -p hrp hrp.elf
		cmd.exe /C c:\\Xilinx\\SDK\\2018.2\\bin\\xsct.bat -interactive ./jtag.tcl ./ps7_init.tcl ./hrp.elf 
