...
/*
 *  【改変 2026-07-02】stk_top/stk_bottom の取り違えを修正（ASP glue と同一バグ）。
 *  hrp3 arm_m の実使用（activate_context: カーネルドメインの初期 SP =
 *  tskinictxb.stk_bottom - 8）と生成 cfg（{ sstk 先頭, 先頭+size, ustk, ustksz }）
 *  より，stk_top＝システムスタック先頭番地（低位）・stk_bottom＝末尾番地（高位）。
 */
size_t
