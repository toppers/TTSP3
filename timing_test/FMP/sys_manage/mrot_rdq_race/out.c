/*
 *  FMP mrot_rdq dispatch race — 案3（ストレスループ）
 *
 *  目的: sys_manage.c mrot_rdq L273 の多コアレース分岐
 *        `if (p_selftsk != p_my_pcb->p_schedtsk){release_glock;dispatch;goto unlock_and_exit;}`
 *        を 2コア・ストレスループで到達させる。
 *
 *  シナリオ:
 *    PE1 : MAIN_TASK : mrot_rdq(1, TPRI_SELF)（PE1 の ready queue を回す）を密ループ
 *    PE2 : RACER_TASK: sus_tsk(MAIN_TASK)/rsm_tsk(MAIN_TASK) を done まで密ループ
 *    → RACER が MAIN を一時 non-runnable にして PE1 の p_schedtsk を変えると L273 真。
 *
...
#define SELF_PRCID  1          /* MAIN は CLS_ALL_PRC1（PE1）で動作 */
...
void main_task(intptr_t exinf)
{
	...
	ttsp_initialize_test_lib();
	syslog_0(LOG_NOTICE, "FMP_mrot_rdq_race: Start");

	ttsp_check_point(1);

	for (i = 0; i < LOOP_N; i++) {
		(void) mrot_rdq(SELF_PRCID, TPRI_SELF);
	}
	...
	syslog_0(LOG_NOTICE, "FMP_mrot_rdq_race: loop done");
	ttsp_check_finish(3);
}
