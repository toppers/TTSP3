...
| `api_test/HRMP/task_term/ter_tsk/*_HM_ex.yaml`（80ファイル） | 改変 | HRP3 3.3→3.4 アクセス許可仕様対応（層2・HRMP）：HRP と同根。access3 に非KERNEL付与154件→access2 へ入替。移行後 E_OACV 早期終了が大幅解消し、HRMP line 91.0%/branch 81.3% を記録（ベースライン比+15.6pp/+17.6pp）。残E_OACVは loc_spn/try_spn（スピンロック・別層） | 済（2026-06-09） |
