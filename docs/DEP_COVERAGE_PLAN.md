...
|---|---|---|---|---|
| ASP3 | 単一コア | 81.4% / 65.0% | **87.8% / 72.5%** | 作成済（step1/2/Tier1） |
...
- 残 32 行：`exc_handler_2/3`+`ret_exc_r`（12＝Tier1.5＝非タスク文脈例外＋復帰時ディスパッチで
  到達余地）／`pabort`/`fiq`/`dabort_handler_1`/`svc_handler`（19＝**ASP の構造的上限**＝保護
  フォルト/FIQ/SVC で踏めない）／`sil_dly_nse1`（1）。ASP の現実的上限は概ね **90% 前後**。
