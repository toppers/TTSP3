...
|---|---|---|---|---|
| ASP3 | 単一コア | 81.4% / 65.0% | **90.4% / 75.0%** | 作成済（step1/2/Tier1/1.5） |
...
- 残 24 行＝**ASP の構造的上限**：`pabort`(5)/`fiq`(5)/`dabort_handler_1`(5)/`svc_handler`(4)＝
  保護フォルト/FIQ/SVC で踏めない、`exc_handler_2`(4)＝ISR からのネスト例外が必要（リスク高）、
  `sil_dly_nse1`(1)。**ASP の現実的上限 ≈ 90%** に到達（残りはアーキ/プロファイル本質的制約）。
