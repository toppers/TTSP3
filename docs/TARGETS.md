...
| ASP | mps2_an505_gcc | QEMU `mps2-an505`（semihosting・**パッチ不要**） | 1 | true | NVIC | `udf #0`＝UsageFault | ⚠ ✗exc/✅int/✅timer（要タイマ停止モードパッチ・注B） | **Cortex-M33/ARMv8-M**。被テストカーネル＝`~/TOPPERS/asp3_tz_work/asp3_3.7`（**main ベースライン**・注B）。per-case **1807/1813 PASS（99.7%・要パッチ）**／素 1293。SIL All passed（`MPS2_API_STATUS.md`） |
