...
| HRP | mps2_an505_gcc | QEMU `mps2-an505`（semihosting・**パッチ不要**） | 1 | true | NVIC | `udf #0`＝UsageFault | ⚠ ✗exc／✅int／✅timer（要タイマ停止モードパッチ） | Cortex-M33＋MPU（8リージョン＝1ドメイン最大5）。per-case **1544/1601 PASS（96.4%・要カーネルパッチ2件＋glue修正）**／旧 758（残 57＝スイート課題 56＋MPU 1。`MPS2_API_STATUS.md`）。`ttsp_check_point` に M-profile 分岐（`TTSP_SILLOC_NO_SVC`） |
...
- **注B（ASP mps2 check_library・2026-07-02 実測）**：int=✅ CP1-12 All passed（QEMU）。
  timer=素では✗ → **タイマ停止モードパッチ**（`docs/patches/asp3-mps2_an505-target_timer-test-stop-mode.patch`）
  で✅ All passed（api per-case も 1293→1807/1813）。exc=✗ ビルド不可
