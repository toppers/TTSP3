...
- **ホワイトボックステストは本トラック（md移行）を待たず、当面は現状の `.txt` シートを更新して進める**
  （WHITEBOX_PLAN.md §6/§6.2）。md 移行時に `.txt` の「2. 構造ベースのホワイトボックステスト」節も
  一緒に md 化し、節目を front-matter `changelog` にノード化する。
