...
### P3. TTG の 3.7 生成対応（残）← **現状トリガされない条件付き将来作業（着手不要）**
- 現状 TA_EDGE/int_trigger_atr（POSIX用）は対応済。zybo生成は緑。
- 本項は「P2 で新機能テストを足す場合に、対応する生成ロジック（サブ優先度の静的API
  `ENA_SPR`／`chg_spr` 呼出し生成等。サブ優先度有効時のみ生成する条件分岐＝
  int_trigger_atr と同手法）を追加する」もの。
- **2026-06-08 確定：P2 は不要（FMP はサブ優先度テスト 65本＋`ENA_SPR` で既存充足、
  ASP3/HRP3 は非搭載＝対象外）。よって P3 も連動して発生しない**。zybo×ASP/FMP の
  緑維持に TTG の追加生成対応は不要。
- 実際に必要となるのは次を選択した時のみ：
  1. ASP3 にサブ優先度**拡張**テストを新規作成する（要 subprio カーネル変種の供給＝判断ゲート G2）
  2. HRMP3 のサブ優先度テストを立ち上げる（HRMP3 後回し方針で保留）
  3. `EXINF` が `intptr_t` 以外の型の新ターゲットを追加する（TTG生成の `intptr_t exinf` を修正）
