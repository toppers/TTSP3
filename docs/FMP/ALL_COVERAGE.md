...
> 対象: FMP3 3.4.0 `kernel/`、APIオートコード 20グループ + check_library + WBテスト8本（計31ディレクトリ, union集計）
...
**分岐カバレッジ（C1）: 1545/1597 = 96.7%**（`all` モード、union集計、-O2+インライン抑制・2026-06-10 再計測）
行カバレッジ（C0）: 3535/3621 = 97.6%

> **2026-06-10 再計測で現状化**（31 dirs union）。旧値 1520/1597=95.2% から **+25 分岐**。内訳：
...
> - 残未到達 52（うち約 17 が interrupt.c・構造的dead/check_intno_cfg複合/chg_ipm timing-affinity）。

- `bb` モード（WBテストなし）: 1533/1597 = 96.0%（2026-06-10 再計測。WB寄与は all−bb = **+12 分岐**）→ [`BB_COVERAGE.md`](BB_COVERAGE.md)
- WBテスト8本（`alarm_W-a` / `cyclic_W-a` / `xsns_dpn_W-a` / `xsns_dpn_W-b` / `time_event_W-a` / `W-b` / **`chg_spr_W-a`** / **`subprio_head_W-a`**）が分岐を追加（後2本はサブ優先度の local dispatch / queue_insert_subprio_head 走査）→ 詳細は [`BB_UNREACHABLE.md`](BB_UNREACHABLE.md) 第1部・§4
...
| **TOTAL** | **3535/3621 (97.6%)** | **1545/1597 (96.7%)** | **52** |

> インライン抑制により分母が 1677→1597 に正規化（wait.h 64→14、task.c 92→78、dataqueue 136→132 等）。残存 **52** 分岐の分類・到達不能理由は [`BB_UNREACHABLE.md`](BB_UNREACHABLE.md) 第2部 / [`TIMING_TEST.md`](TIMING_TEST.md) を参照（2026-06-10 再計測：サブ優先度BB/WB ＋ interrupt負テストBB を反映）。
...
FMP の残存 **52** 分岐の主要因はマルチコア遅延ディスパッチパス（timing 系・[`TIMING_TEST.md`](TIMING_TEST.md)）・interrupt.c の `VALID_INTNO` 構造的dead＋`check_intno_cfg` 複合＋chg_ipm（L383 timing/L392 affinity）。サブ優先度機能（`TA_SUBPRI`/`ENA_SPR`）と割込み番号バリデーション異常系は 2026-06-10 のBB/WB追加で到達済み。FMP 固有のマルチコア協調処理・サブ優先度機能が ASP に対し分岐数を +218 増やしている。マルチコアレース分岐は案3（2コアストレスループ）で到達可能なことが実証済み。
