...
> 対象: FMP3 3.4.0 `kernel/`、APIオートコード 20グループ + check_library + WBテスト10本（計33ディレクトリ, union集計）
...
**分岐カバレッジ（C1）: 1550/1597 = 97.1%**（`all` モード、union集計、-O2+インライン抑制・2026-06-10 再計測）
行カバレッジ（C0）: 3539/3621 = 97.7%

> **2026-06-10 再計測で現状化**（33 dirs union）。旧値 1520/1597=95.2% から **+30 分岐**。内訳：
...
> - **単一TM-processor変種WB（`tepp1_W-a`/`tepp1_W-b`・`-DTOPPERS_TEPP_PRC=0x1`）：time_event.c §5-c 66→70（+4）・
>   startup.c §9-b 29→30（+1）**。`BB_UNREACHABLE.md §5-c` / `COVERAGE_RAISE_PLAN.md` Method B 参照。
> - 残未到達 47（うち約 17 が interrupt.c・構造的dead/check_intno_cfg複合/chg_ipm timing-affinity）。

- `bb` モード（WBテストなし）: 1533/1597 = 96.0%（2026-06-10 再計測。WB寄与は all−bb = **+17 分岐**）→ [`BB_COVERAGE.md`](BB_COVERAGE.md)
- WBテスト10本（`alarm_W-a` / `cyclic_W-a` / `xsns_dpn_W-a` / `xsns_dpn_W-b` / `time_event_W-a` / `W-b` / `chg_spr_W-a` / `subprio_head_W-a` / **`tepp1_W-a`** / **`tepp1_W-b`**）が分岐を追加（`tepp1_W-a` は単一TM-processor構成で §5-c/§9-b）→ 詳細は [`BB_UNREACHABLE.md`](BB_UNREACHABLE.md) 第1部・§4・§5-c
...
| time_event.c | 181/185 (97.8%) | 70/76 (92.1%) | 6 |
...
| **TOTAL** | **3539/3621 (97.7%)** | **1550/1597 (97.1%)** | **47** |

> インライン抑制により分母が 1677→1597 に正規化（wait.h 64→14、task.c 92→78、dataqueue 136→132 等）。残存 **47** 分岐の分類・到達不能理由は [`BB_UNREACHABLE.md`](BB_UNREACHABLE.md) 第2部 / [`TIMING_TEST.md`](TIMING_TEST.md) を参照（2026-06-10 再計測：サブ優先度BB/WB ＋ interrupt負テストBB ＋ 単一TM-processor変種 tepp1_W-a を反映）。
