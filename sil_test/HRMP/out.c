...
PRC_INFO prc_info[] = {
#ifdef TTSP_INTNO_B
#ifdef TTSP_SIL_TEST_ATT_ISR /* 【改変】ATT_ISR非対応のためISR文脈無効化 */
...
void main_task(intptr_t exinf) {
	...
	/*
	 * タスク例外処理ルーチン
	 * 【TTSP3向け改変 2026-06-14】第3世代カーネルはタスク例外処理機能を持たないため
	 * 当該文脈の SIL テストは対象外。CP番号の連続性のため CP3/CP4 はプレースホルダ。
	 */
	syslog_1(LOG_NOTICE, "PE %d : === test from TASK EXCEPTION: skipped (no task exception in 3rd gen) ===", prcid);
 ...
 * タスク例外機能が無いため削除（DEF_TEX も out.cfg から除去）． */
