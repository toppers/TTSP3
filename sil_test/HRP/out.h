...
/* 【TTSP3向け改変 2026-06-14・HRP版】二重インクルード防止のためインクルードガードを追加
 * （HRP の cfg 生成経路では out.h が複数回展開され enum/宣言が再定義エラーになるため）． */
...
/* 【TTSP3向け改変 2026-06-14・HRP版】inthdr/isr の extern 宣言を無条件化．
 * HRP の cfg 由来 kernel_cfg.c（割込みベクタ表）は TTSP_INTNO_B 未定義の文脈で
 * out.h を取り込むため，#ifdef 下だと宣言が見えず inthdr undeclared になる．
 * 宣言は無条件でも無害（実体・使用は out.c/out.cfg 側の #ifdef のまま）． */
