...
#  Copyright (C) 2009-2020 by FUJI SOFT INCORPORATED
# 
...
#  の責任を負わない．
# 
#  $Id: common.sh 70 2020-02-04 09:29:02Z fujisft-shigihara $
# 
...
ERR_FILE_OR_DIVNUM_INVALID="temp manifest or divide number is invalid"
...
variation_check()
{
	...
	# プロファイルのチェック
	variation_name=$VARI_PROFILE
	get_variation_value
	if [ $variation_value ]
	then
		chk=`echo $variation_value | grep $PROFILE_NAME | wc -l`
		if [ $chk -eq 0 ]
		then
			vari_flg=1
			return
	...
	if [ $variation_value ]
	then
		chk=`echo $variation_value | grep $IRC_ARCH | wc -l`
		if [ $chk -eq 0 ]
		then
			vari_flg=1
			return
	...
	then
		...
		case $rule in
			$RULE_DEPEND)
			 ...
			 echo "'make depend' is obsoleted in TOPPERS third generation kernel"
			 ;;
			$RULE_BUILD)
			 ...
			 ;;
			$RULE_CLEAN)
			 make clean $MAKE_OPT KERNEL_COBJS="$KERNEL_COBJS_COMMON $KERNEL_COBJS_TARGET" APPL_COBJS="$APPL_COBJS_COMMON $APPL_COBJS_TARGET"
			 ;;
			$RULE_REALCLEAM)
			 make realclean $MAKE_OPT KERNEL_COBJS="$KERNEL_COBJS_COMMON $KERNEL_COBJS_TARGET" APPL_COBJS="$APPL_COBJS_COMMON $APPL_COBJS_TARGET"
			 ;;
		esac
		cd $TTSP_DIR
	fi
