...
#  Copyright (C) 2009-2018 by FUJI SOFT INCORPORATED
# 
...
#  の責任を負わない．
# 
#  $Id: api_test.sh 88 2020-07-20 06:56:44Z ertl-honda $
# 
...
echo -n " $INPUT_NO "
		...
		case ${key} in
			...
			2)
			 ...
			 ruby ${g_tree_level}${OS_PATH}/configure.rb $CONFIG_TEST_PROGRAM "$INCLUDE_DIR" -U "$TEST_LIB_FILE" &> /dev/null
			 if [ -f $TTSP_DIR/library/$PROFILE_NAME/test/out.cdl ]
			 then
				cp -pf $TTSP_DIR/library/$PROFILE_NAME/test/out.cdl .
			 fi
			 cd $TTSP_DIR
			 ;;
			3)
...
make_manifest_for_tesry()
{
	...
	do
		...
		then
			...
			find $api_test_dir/ -name "*.yaml" | sort >> temp_aut
			flg=1
		fi
	done
	if [ $flg = 1 ]
	then
		...
		case $kind in
			...
			$API_DIV)
			 ...
			 if [ "$AVERAGE_MANIFEST" == "true" ]
			 then
			 	ruby $TTSP_DIR/scripts/average_manifest.rb temp_aut $div_num > .tmp
			 	if [ $? -ne 0 ]
			 	then
			 		echo "$ERR_FILE_OR_DIVNUM_INVALID"
			 		cd $TTSP_DIR
			 		return
			 	fi
			 	mv -f .tmp temp_aut
...
make_manifest_for_scr_and_err()
{
	...
	do
		...
		then
			api_test_dir=`get_realpath ../../$dir`
			find $api_test_dir/ -name "*.cfg" -print | sort >> $temp
			flg=1
		fi
	done
	if [ $flg = 1 ]
	...
	do
		...
		then
			...
			then
				...
				cp -rp temp_dir_aut/* ./$pre_dir_name\_$cnt
				cnt=`expr $cnt + 1 `
				flg=1
			fi
		fi
...
execute_ttg_for_no_manifest()
{
	...
	do
		...
		then
			...
			find $yaml_path/ -name "*.yaml" | sort > temp_tesry
...
compare_error_code()
{
	...
	make realclean $MAKE_OPT KERNEL_COBJS="$KERNEL_COBJS_COMMON $KERNEL_COBJS_TARGET" APPL_COBJS="$APPL_COBJS_COMMON $APPL_COBJS_TARGET" &> /dev/null
	# make depend $MAKE_OPT KERNEL_COBJS="$KERNEL_COBJS_COMMON $KERNEL_COBJS_TARGET" APPL_COBJS="$APPL_COBJS_COMMON $APPL_COBJS_TARGET" 1> /dev/null 2> $RESULT_MAKE_DEPEND
	...
	#	continue
	# fi
	make $MAKE_OPT KERNEL_COBJS="$KERNEL_COBJS_COMMON $KERNEL_COBJS_TARGET" APPL_COBJS="$APPL_COBJS_COMMON $APPL_COBJS_TARGET" 1> /dev/null 2> $RESULT_MAKE
	chk=`grep "$err_code" $RESULT_MAKE | wc -l`
	if [ "$chk" -ne 0 ]
	then
		echo "$dir_name $TEST_OK" | tee -a ../$RESULT_CONF_ERR
	else
...
continuous_execute_for_specified_yaml()
{
	...
	do
		...
		then
			...
			find $yaml_path/ -name "*.yaml" | sort > temp_ttg
...
then
	...
	then
	  TTG_OPT=`echo "-f $TTG_OPT --prc_num $PROCESSOR_NUM --out_file_name $APPLI_NAME \
	                 --func_time $FUNC_TIME --func_interrupt $FUNC_INTERRUPT --func_exception $FUNC_EXCEPTION --irc_arch $IRC_ARCH"`
	elif [ $PROFILE_NAME = "HRP" ]
	then
	  TTG_OPT=`echo "-h $TTG_OPT --out_file_name $APPLI_NAME --func_time $FUNC_TIME --func_interrupt $FUNC_INTERRUPT --func_exception $FUNC_EXCEPTION"`
	else
	  TTG_OPT=`echo "-H $TTG_OPT --prc_num $PROCESSOR_NUM --out_file_name $APPLI_NAME \
	                 --func_time $FUNC_TIME --func_interrupt $FUNC_INTERRUPT --func_exception $FUNC_EXCEPTION --irc_arch $IRC_ARCH"`
	fi
