...
#
#  FMP3側の GCOV 対応（ENABLE_GCOV=true，要 fmp3 側 Makefile.target/
#  zybo_z7.ld/target_kernel_impl.c の対応版）を使い，計装ビルド→QEMU実行→
#  gcov集計（行＋分岐C1）を行う．結果は scripts/ttsp_gcov_report.py で
#  /fmp3/kernel/ にフィルタして統合する．
#
#  使い方:
...
echo ""
python3 scripts/ttsp_gcov_report.py $dirs
