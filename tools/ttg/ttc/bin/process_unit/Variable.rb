...
#  の責任を負わない．
#
#  $Id: Variable.rb 60 2019-12-20 01:14:37Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class Variable
    ...
    def check_attribute_value()
      ...
      check_attribute_type(TSR_PRM_VAR_ARRAY_SZ, @nArraySize, Integer, true, @aPath)
    ...
    def object_check(bIsPre)
      ...
      ### T3_VAR004: 変数が配列の場合に値のフォーマットが不正
      if (!@nArraySize.nil?() && !@snValue.nil?())
        if (@snValue !~ /^\{\w[\w\s,]*\}$/)
          aErrors.push(YamlError.new("T3_VAR004: " + ERR_VAR_INVALID_VALUE_FORMAT, @aPath))
        else
          sTemp = @snValue.gsub("{", "").gsub("}", "")
          @aArrayData = sTemp.split(",")
        end
      end
    ...
    def convert_macro()
      ...
      # 配列の要素数
      @nArraySize = calc_all_expr(@nArraySize, hMacro)
      # メンバ
      unless (@hMember.nil?())
    ...
    def complement(cPrevVar)
      ...
      # 配列の要素数
      unless (is_specified?(TSR_PRM_VAR_ARRAY_SZ))
        @nArraySize = safe_dup(cPrevVar.nArraySize)
      end
      # 値
    ...
    def dup()
      ...
      # 配列の要素数複製
      cVar.nArraySize = safe_dup(@nArraySize)
      # 値複製
    ...
    def to_yaml()
      ...
      # arraysz
      if (!@nArraySize.nil?() || is_specified?(TSR_PRM_VAR_ARRAY_SZ))
        hYaml[TSR_PRM_VAR_ARRAY_SZ] = @nArraySize
      end
      # value
