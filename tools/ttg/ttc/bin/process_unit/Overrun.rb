...
module CommonModule
  ...
  class Overrun < ProcessUnit
    ...
    def object_check(bIsPre)
      ...
      ### T3_OVR001: ovrstatにTOVR_STPが指定されていない(制限事項)
      if (@hState[TSR_PRM_STATE] != TSR_STT_TOVR_STP)
        aErrors.push(YamlError.new("T3_OVR001: " + "[constraint] Overrun Handler is not supported now", @aPath))
      end
