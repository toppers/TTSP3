...
#  の責任を負わない．
#
#  $Id: Task.rb 47 2019-10-31 02:49:45Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class Task < ProcessUnit
    ...
    def object_check(bIsPre)
      ...
      if (@cConf.is_hrp?())
        ### T3_TSK009: domainに"NONE"が指定されている
        if (@hState[TSR_PRM_DOMAIN] == TSR_STT_DOM_NONE)
          aErrors.push(YamlError.new("T3_TSK009: " + ERR_SET_OUT_OF_DOMAIN, @aPath))
        end
      end

      ### T3_TSK010: pre_conditionでdisterがfalseの場合にrasterにtrueが指定されている
      if ((bIsPre == true) && (@hState[TSR_PRM_DISTER] == false) && (@hState[TSR_PRM_RASTER] == true))
        aErrors.push(YamlError.new("T3_TSK010: " + ERR_SET_OUT_OF_RASTER, @aPath))
      end
    ...
    def complement_init_object_info()
      ...
      # tskpri
      if (!is_specified?(TSR_PRM_TSKPRI))
        @hState[TSR_PRM_TSKPRI]  = hMacro["TSK_PRI_MID"]
      end

      # tskbpri
      if (!is_specified?(TSR_PRM_TSKBPRI))
        @hState[TSR_PRM_TSKBPRI] = @hState[TSR_PRM_TSKPRI]
      end

      # itskpri
      if (!is_specified?(TSR_PRM_ITSKPRI))
        @hState[TSR_PRM_ITSKPRI] = @hState[TSR_PRM_TSKBPRI]
      end
      ...
      # bootcnt
      if (!is_specified?(TSR_PRM_BOOTCNT))
        @hState[TSR_PRM_BOOTCNT] = 0
      end

      # raster
      if (!is_specified?(TSR_PRM_RASTER))
        @hState[TSR_PRM_RASTER] = false
      end

      # dister
      if (!is_specified?(TSR_PRM_DISTER))
        @hState[TSR_PRM_DISTER] = false
      end

      # stksz
      if (!is_specified?(TSR_PRM_STKSZ))
        @hState[TSR_PRM_STKSZ] = "TTSP_TASK_STACK_SIZE"
      end

      if (@cConf.is_hrp?())
        # domain
        if (!is_specified?(TSR_PRM_DOMAIN))
          # タスクが所属するドメインが指定されていない場合，デフォルト値を設定する
          @hState[TSR_PRM_DOMAIN] = hMacro["DOM_TASK"]
        end

        # sstksz
        if (!is_specified?(TSR_PRM_SSTKSZ))
          @hState[TSR_PRM_SSTKSZ] = "TTSP_TASK_STACK_SIZE"
        end
      end
    end
    ...
    def complement(cPrevObjectInfo)
      ...
      # 補完例外(5)
      if (is_specified?(TSR_PRM_TSKPRI) && !is_specified?(TSR_PRM_TSKBPRI))
        @hState[TSR_PRM_TSKBPRI] = @hState[TSR_PRM_TSKPRI]
      end
    end
    ...
    #=================================================================
    def is_complemented_user_domain?()
      hMacro = @cConf.get_macro()

      if (!is_specified?(TSR_PRM_DOMAIN) && (hMacro["DOM_TASK"] != TSR_STT_DOM_KERNEL))
