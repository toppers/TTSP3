...
#  Copyright (C) 2012-2019 by FUJI SOFT INCORPORATED
#
...
#  の責任を負わない．
#
#  $Id: ProcessUnit.rb 60 2019-12-20 01:14:37Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class ProcessUnit
        ...
        begin
          ...
          if (is_specified?(sAtr))
            ...
            when TSR_PRM_LEFTTMO, TSR_PRM_LEFTTIM, TSR_PRM_EXINF, TSR_PRM_BOOTCNT, TSR_PRM_PORDER,
                 TSR_PRM_CYCPHS, TSR_PRM_CYCTIM, TSR_PRM_PNDPTN, TSR_PRM_TEXPTN, TSR_PRM_SUBPRI
            ...
            # 文字列
            when TSR_PRM_CLASS, TSR_PRM_TASK, TSR_PRM_SPINID, TSR_PRM_DOMAIN, TSR_PRM_CALLER
            ...
            # 0以上の整数か文字列
            when TSR_PRM_ACTPRC, TSR_PRM_EXCNO, TSR_PRM_INHNO, TSR_PRM_INTNO, TSR_PRM_FNCD,
                 TSR_PRM_ACCESS1, TSR_PRM_ACCESS2, TSR_PRM_ACCESS3, TSR_PRM_ACCESS4,
                 TSR_PRM_STKSZ, TSR_PRM_SSTKSZ, TSR_PRM_STK, TSR_PRM_SSTK,
                 TSR_PRM_NFYINFO1, TSR_PRM_NFYINFO2, TSR_PRM_ENFYINFO1, TSR_PRM_ENFYINFO2
            ...
            # 優先度
            when TSR_PRM_ITSKPRI, TSR_PRM_TSKBPRI, TSR_PRM_TSKPRI, TSR_PRM_ISRPRI
            ...
            # 配列
            when TSR_PRM_ARGS
              check_attribute_type(sAtr, val, Array, false, @aPath)

            # 配列かNil
            when TSR_PRM_LOCMTX
              check_attribute_type(sAtr, val, Array, true, @aPath)
            ...
            when TSR_PRM_HDLSTAT
              if (@cConf.is_asp?())
                ...
                aList = GRP_ENUM_HDLSTAT_HRMP
              elsif (@cConf.is_fmp?())
                aList = GRP_ENUM_HDLSTAT_FMP
              elsif (@cConf.is_hrp?())
                aList = GRP_ENUM_HDLSTAT_HRP
              else
            ...
            # state
            when TSR_PRM_TSKSTAT, TSR_PRM_ALMSTAT, TSR_PRM_CYCSTAT, TSR_PRM_TEXSTAT, TSR_PRM_INTSTAT, TSR_PRM_OVRSTAT
              # TESRYでの値に戻す
              ...
              if (@cConf.is_asp?())
                ...
                aList = GRP_OBJECT_STATE_AVAILABLE_HRMP
              elsif (@cConf.is_fmp?())
                aList = GRP_OBJECT_STATE_AVAILABLE_FMP
              elsif (@cConf.is_hrp?())
                aList = GRP_OBJECT_STATE_AVAILABLE_HRP
              else
                abort(ERR_MSG % [__FILE__, __LINE__])
              end
              check_attribute_type(sAtr, val, String, false, @aPath)
            ...
            # nfytype
            when TSR_PRM_NFTTYPE
              check_attribute_type(sAtr, val, String, false, @aPath)
              check_attribute_enum(sAtr, val, GRP_TNFY_TYPE, @aPath)

            # enfytype
            when TSR_PRM_ENFYTYPE
              check_attribute_type(sAtr, val, String, false, @aPath)
              check_attribute_enum(sAtr, val, GRP_TENFY_TYPE, @aPath)
    ...
    def object_check(bIsPre = false)
      ...
      if ((bIsPre == true) && @cConf.is_hrp?())
        ...
        if (!((@hState[TSR_PRM_ACCESS1].nil?() && @hState[TSR_PRM_ACCESS2].nil?() &&
          ...
          case @sObjectType
          when TSR_OBJ_TASK
            aErrors.push(YamlError.new("T3_TSK008: " + ERR_NO_ALL_ACCESS_SET, @aPath))
          when TSR_OBJ_ALARM
            aErrors.push(YamlError.new("T3_ALM003: " + ERR_NO_ALL_ACCESS_SET, @aPath))
          when TSR_OBJ_CYCLE
            aErrors.push(YamlError.new("T3_CYC004: " + ERR_NO_ALL_ACCESS_SET, @aPath))
          else
            abort(ERR_MSG % [__FILE__, __LINE__])
          end
        end
      ...
      # タイムイベント通知，エラー通知のチェック
      if ((@sObjectType == TSR_OBJ_ALARM) || (@sObjectType == TSR_OBJ_CYCLE))
        ### T3_ALM004，T3_CYC005: nfytypeがTNFY_HANDLERの場合に，hdlstatが指定されていない
        if ((@hState[TSR_PRM_NFTTYPE] == TSR_STT_TNFY_HANDLER) && @hState[TSR_PRM_HDLSTAT].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM004: " + ERR_NO_HDLSTAT, TSR_OBJ_ALARM)
          else
            sErr = sprintf("T3_CYC005: " + ERR_NO_HDLSTAT, TSR_OBJ_CYCLE)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        ### T3_ALM005，T3_CYC006: nfytypeがTNFY_HANDLERでない場合に，hdlstat，bootcnt，spinid，varが指定されている
        if ((@hState[TSR_PRM_NFTTYPE] != TSR_STT_TNFY_HANDLER) &&
            (!@hState[TSR_PRM_HDLSTAT].nil?() || is_specified?(TSR_PRM_BOOTCNT) ||
             !@hState[TSR_PRM_SPINID].nil?() || !@hState[TSR_PRM_VAR].nil?()))
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM005: " + ERR_INVALID_PARAM_NOT_HANDLER, TSR_OBJ_ALARM)
          else
            sErr = sprintf("T3_CYC006: " + ERR_INVALID_PARAM_NOT_HANDLER, TSR_OBJ_CYCLE)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        # エラーチェック完了後に，ハンドラ以外の場合は停止状態としておく
        if (@hState[TSR_PRM_HDLSTAT].nil?())
          @hState[TSR_PRM_HDLSTAT] = TSR_STT_STP
        end

        ### T3_ALM006，T3_CYC007: nfytypeがTNFY_SETVAR，TNFY_SETFLG，TNFY_SNDDTQの場合に，nfy_info2が指定されていない
        aNeedNfyInfo2List = [TSR_STT_TNFY_SETVAR, TSR_STT_TNFY_SETFLG, TSR_STT_TNFY_SNDDTQ]
        if (aNeedNfyInfo2List.include?(@hState[TSR_PRM_NFTTYPE]) && @hState[TSR_PRM_NFYINFO2].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM006: " + ERR_NO_INFO2, TSR_OBJ_ALARM, aNeedNfyInfo2List.join("\" or \""), TSR_PRM_NFYINFO2)
          else
            sErr = sprintf("T3_CYC007: " + ERR_NO_INFO2, TSR_OBJ_CYCLE, aNeedNfyInfo2List.join("\" or \""), TSR_PRM_NFYINFO2)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        ### T3_ALM007，T3_CYC008: nfytypeがTNFY_SETVAR，TNFY_SETFLG，TNFY_SNDDTQ以外の場合に，nfy_info2が指定されている
        if (!aNeedNfyInfo2List.include?(@hState[TSR_PRM_NFTTYPE]) && !@hState[TSR_PRM_NFYINFO2].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM007: " + ERR_INVALID_INFO2, TSR_OBJ_ALARM, aNeedNfyInfo2List.join("\" or \""), TSR_PRM_NFYINFO2)
          else
            sErr = sprintf("T3_CYC008: " + ERR_INVALID_INFO2, TSR_OBJ_CYCLE, aNeedNfyInfo2List.join("\" or \""), TSR_PRM_NFYINFO2)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        ### T3_ALM008，T3_CYC009: nfytypeがTNFY_HANDLER，TNFY_SETVAR，TNFY_INCVARの場合に，enfytypeが指定されている
        aNoEnfyTypeList = [TSR_STT_TNFY_HANDLER, TSR_STT_TNFY_SETVAR, TSR_STT_TNFY_INCVAR]
        if (aNoEnfyTypeList.include?(@hState[TSR_PRM_NFTTYPE]) && !@hState[TSR_PRM_ENFYTYPE].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM008: " + ERR_INVALID_ENFYTYPE, TSR_OBJ_ALARM, aNoEnfyTypeList.join("\" or \""), TSR_PRM_ENFYTYPE)
          else
            sErr = sprintf("T3_CYC009: " + ERR_INVALID_ENFYTYPE, TSR_OBJ_CYCLE, aNoEnfyTypeList.join("\" or \""), TSR_PRM_ENFYTYPE)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        ### T3_ALM009，T3_CYC010: enfytypeが指定されている場合に，enfy_info1が指定されていない
        if (!@hState[TSR_PRM_ENFYTYPE].nil?() && @hState[TSR_PRM_ENFYINFO1].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM009: " + ERR_NO_INFO2, TSR_OBJ_ALARM, TSR_PRM_ENFYTYPE, TSR_PRM_ENFYINFO1)
          else
            sErr = sprintf("T3_CYC010: " + ERR_NO_INFO2, TSR_OBJ_CYCLE, TSR_PRM_ENFYTYPE, TSR_PRM_ENFYINFO1)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        ### T3_ALM010，T3_CYC011: enfytypeがTENFY_SETFLGの場合に，nfy_info2が指定されていない
        if ((@hState[TSR_PRM_ENFYTYPE] == TSR_STT_TENFY_SETFLG) && @hState[TSR_PRM_ENFYINFO2].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM010: " + ERR_NO_INFO2, TSR_OBJ_ALARM, TSR_STT_TENFY_SETFLG, TSR_PRM_ENFYINFO2)
          else
            sErr = sprintf("T3_CYC011: " + ERR_NO_INFO2, TSR_OBJ_CYCLE, TSR_STT_TENFY_SETFLG, TSR_PRM_ENFYINFO2)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end

        ### T3_ALM011，T3_CYC012: enfytypeがTENFY_SETFLG以外の場合に，nfy_info2が指定されている
        if ((@hState[TSR_PRM_ENFYTYPE] != TSR_STT_TENFY_SETFLG) && !@hState[TSR_PRM_ENFYINFO2].nil?())
          if (@sObjectType == TSR_OBJ_ALARM)
            sErr = sprintf("T3_ALM011: " + ERR_INVALID_INFO2, TSR_OBJ_ALARM, TSR_STT_TENFY_SETFLG, TSR_PRM_ENFYINFO2)
          else
            sErr = sprintf("T3_CYC012: " + ERR_INVALID_INFO2, TSR_OBJ_CYCLE, TSR_STT_TENFY_SETFLG, TSR_PRM_ENFYINFO2)
          end
          aErrors.push(YamlError.new(sErr, @aPath))
        end
      end
    ...
    def complement_init_object_info()
      ...
      if (@cConf.is_fmp?())
        ...
        if ((@sObjectType == TSR_OBJ_INIRTN || @sObjectType == TSR_OBJ_TERRTN) && @hState[TSR_PRM_GLOBAL] == true)
          ...
          # class
          unless (is_specified?(TSR_PRM_CLASS))
            if ((@sObjectType == TSR_OBJ_ISR) || (@sObjectType == TSR_OBJ_INTHDR))
      ...
      if (@cConf.is_hrp?())
        ...
        if (!is_specified?(TSR_PRM_DOMAIN))
          # タスク以外の処理単位はすべてカーネルドメインに所属する
          @hState[TSR_PRM_DOMAIN] = TTG_MAIN_DOMAIN
        end
      end
    end
    ...
    def alias(hAlias)
      ...
      # locmtx
      unless (@hState[TSR_PRM_LOCMTX].nil?())
        aTmp = @hState[TSR_PRM_LOCMTX]
        @hState[TSR_PRM_LOCMTX] = []
        aTmp.each{|sTask|
          @hState[TSR_PRM_LOCMTX].push(alias_replace(sTask, hAlias))
        }
      end
    ...
    def get_alias(sTestID, nNum = nil)
      ...
      # オーバランハンドラの場合，ラベルのみへ置き換える(オーバランハンドラはシステムで1つ)
      when TSR_OBJ_OVERRUN
        hResult[@sObjectID] = TTG_LBL_OVERRUN
      # 拡張サービスコールの場合，ラベル+機能コードへ置き換える
      when TSR_OBJ_SVC
        hResult[@sObjectID] = "#{TTG_LBL_SVC}_#{@hState[TSR_PRM_FNCD]}"
      else
