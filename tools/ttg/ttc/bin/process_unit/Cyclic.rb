...
#  の責任を負わない．
#
#  $Id: Cyclic.rb 47 2019-10-31 02:49:45Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class Cyclic < ProcessUnit
    ...
    def object_check(bIsPre)
      ...
      ### T3_CYC002: nfytypeがTNFY_HANDLERの場合で，hdlstatがACTIVATEのときに，cycstatがTCYC_STAでない
      if ((@hState[TSR_PRM_NFTTYPE] == TSR_STT_TNFY_HANDLER) &&
          (@hState[TSR_PRM_HDLSTAT] == TSR_STT_ACTIVATE) && !is_cyc_sta?())
        aErrors.push(YamlError.new("T3_CYC002: " + ERR_NOT_STA_ON_ACTIVATE, @aPath))
      end
      ### T3_CYC003: nfytypeがTNFY_HANDLERの場合で，hdlstatがACTIVATE-waitspinの時にspinidが指定されていない
      if ((@hState[TSR_PRM_NFTTYPE] == TSR_STT_TNFY_HANDLER) &&
          (@hState[TSR_PRM_HDLSTAT] == TSR_STT_A_WAITSPN) && @hState[TSR_PRM_SPINID].nil?())
