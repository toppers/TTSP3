...
#  の責任を負わない．
#
#  $Id: ttc.rb 60 2019-12-20 01:14:37Z fujisft-shigihara $
#
if ($0 == __FILE__)
...
module TTCModule
  ...
  class TTC
    ...
    def pre_file_check()
      ...
      # デバッグ出力用ファイル削除
      if (@cConf.is_debug_mode?())
        aFiles = [TTC_DEBUG_FILE_BEFORE, TTC_DEBUG_FILE_AFTER]
    ...
    def parse_option(aArgs)
      ...
      cOpt = OptionParser.new("Usage: ttg.rb [-a | -f | -h | -H] [options]... [yaml files]...")
      # バージョン情報設定
      cOpt.version = $Version

      # プロファイル設定
      cOpt.on("-a", "asp mode"){
        @cConf.set(CFG_PROFILE, CFG_PROFILE_ASP)
      }
      cOpt.on("-f", "fmp mode"){
        ...
        @cConf.set(CFG_PROFILE, CFG_PROFILE_FMP)
      }
      cOpt.on("-h", "hrp mode"){
        if (@cConf.is_profile_set?())
          puts cOpt.help()
        ...
        @cConf.set(CFG_PROFILE, CFG_PROFILE_HRP)
      }
      cOpt.on("-H", "hrmp mode"){
        if (@cConf.is_profile_set?())
          puts cOpt.help()
        ...
        @cConf.set(CFG_PROFILE, CFG_PROFILE_HRMP)
      }
      ...
      cOpt.on("--#{CFG_SUPPORT_CLR_INT} BOOL", "support api \"#{API_CLR_INT}\""){|val|
        @cConf.set(CFG_SUPPORT_CLR_INT, val)
      }
      cOpt.on("--#{CFG_SUPPORT_RAS_INT} BOOL", "support api \"#{API_RAS_INT}\""){|val|
        @cConf.set(CFG_SUPPORT_RAS_INT, val)
      }
      cOpt.on("--#{CFG_SUPPORT_PRB_INT} BOOL", "support api \"#{API_PRB_INT}\""){|val|
        @cConf.set(CFG_SUPPORT_PRB_INT, val)
      }
