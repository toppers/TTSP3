...
#  の責任を負わない．
#
#  $Id: TestScenario.rb 70 2020-02-04 09:29:02Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class TestScenario
      ...
      if (hVariation.is_a?(Hash))
        ...
        hVariation.each{|sAtr, val|
          ...
          when TSR_PRM_GAIN_TIME, TSR_PRM_ENA_EXC_LOCK, TSR_PRM_GCOV_ALL, TSR_PRN_ENA_CHGIPM,
               ...
               TSR_PRM_SUPPORT_CLR_INT, TSR_PRM_SUPPORT_RAS_INT, TSR_PRM_SUPPORT_PRB_INT
          ...
          # プロファイル
          when TSR_PRM_PROFILE
            if (val.is_a?(String))
              val.gsub!(" ", "")
              aTemp = val.split("|")
              aTemp.each{|sProfile|
                if (!["ASP", "FMP", "HRP"].include?(sProfile))
                  sErr = sprintf(ERR_VARIATION_INVALID_PROFILE, sProfile)
                  aErrors.push(YamlError.new(sErr, aNewPath))
                end
              }
            else
              sErr = sprintf(ERR_VARIATION_INVALID_PROFILE, val)
              aErrors.push(YamlError.new(sErr, aNewPath))
            end
    ...
    def condition_check()
      aErrors = []
      begin
        @cPreCondition.condition_check(true)
      rescue TTCMultiError
        aErrors.concat($!.aErrors)
      end
      @aDo_PostCondition.each{|cCondition|
...
=end
      ...
      ### T5_014: pre_conditionもしくは最後のpost_conditionに存在する割込みハンドラ，割込みサービスルーチンで，intstatがTA_ENAINTの場合にintreqにtrueが指定されている
      ...
      aCondition.each{|cCondition|
        ...
        hObjects.each{|sObjectID, cObjectInfo|
          if ((cObjectInfo.hState[TSR_PRM_STATE] == TSR_STT_TA_ENAINT) && (cObjectInfo.hState[TSR_PRM_INTREQ] == true))
            sErr = sprintf("T5_014: " + ERR_SET_OUT_OF_INTREQ, sObjectID)
            aPath = cCondition.get_condition_path() + [sObjectID, TSR_PRM_INTREQ]
            aErrors.push(YamlError.new(sErr, aPath))
          end
        }
      }
    ...
    def variation_check()
      ...
      ### T6_013: APIのclr_intがサポートされていない
      if (@cConf.get_api_support_clr_int() == false && @hVariation[TSR_PRM_SUPPORT_CLR_INT] == true)
        aErrors.push(TTCExcludeError.new("T6_013: " + ERR_VARIATION_NOT_SUPPORT_CLR_INT, :T6_013))
      end

      ### T6_014: APIのras_intがサポートされていない
      if (@cConf.get_api_support_ras_int() == false && @hVariation[TSR_PRM_SUPPORT_RAS_INT] == true)
        aErrors.push(TTCExcludeError.new("T6_014: " + ERR_VARIATION_NOT_SUPPORT_RAS_INT, :T6_014))
      end

      ### T6_015: APIのprb_intがサポートされていない
      if (@cConf.get_api_support_prb_int() == false && @hVariation[TSR_PRM_SUPPORT_PRB_INT] == true)
        aErrors.push(TTCExcludeError.new("T6_015: " + ERR_VARIATION_NOT_SUPPORT_PRB_INT, :T6_015))
      end
...
=end
      ...
      ### T6_012: 対象カーネルでは実行できない
      if (@hVariation.has_key?(TSR_PRM_PROFILE))
        aList = @hVariation[TSR_PRM_PROFILE].gsub(" ", "").split("|")
        bChk = false
        aList.each{|sProfile|
          if ((@cConf.is_asp?() && (sProfile == "ASP")) ||
              (@cConf.is_fmp?() && !@cConf.is_hrp?() && (sProfile == "FMP")) ||
              (@cConf.is_hrp?() && !@cConf.is_fmp?() && (sProfile == "HRP")) ||
              (@cConf.is_hrp?() && @cConf.is_fmp?() && (sProfile == "HRMP")) )
            bChk = true
          end
        }
        if (bChk == false)
          aErrors.push(TTCExcludeError.new("T6_012: " + ERR_VARIATION_NOT_SUPPORT_PROFILE, :T6_012))
        end
      end
      ...
      if (@cConf.is_fmp?())
        ...
        if (!@hVariation[TSR_PRM_IRC_ARCH].nil?())
          aList = @hVariation[TSR_PRM_IRC_ARCH].gsub(" ", "").split("|")
          bChk = false
          aList.each{|sIrcArch|
            if (sIrcArch == @cConf.get_irc_arch())
              bChk = true
              break
            end
          }
      ...
      if (@cConf.is_hrp?())
        ### T6_H001: タスク例外が存在する
        hObjects = @cPreCondition.get_objects_by_type([TSR_OBJ_TASK_EXC])
        if (!hObjects.empty?())
          aErrors.push(TTCExcludeError.new("T6_H001: " + ERR_VARIATION_TASK_EXC_IN_HRP, :T6_010))
        end

        ### T6_H002: メールボックスが存在する
        hObjects = @cPreCondition.get_objects_by_type([TSR_OBJ_MAILBOX])
        if (!hObjects.empty?())
          aErrors.push(TTCExcludeError.new("T6_H002: " + ERR_VARIATION_MAILBOX_IN_HRP, :T6_010))
        end
      end
