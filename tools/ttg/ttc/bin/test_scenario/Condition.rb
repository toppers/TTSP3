...
#  の責任を負わない．
#
#  $Id: Condition.rb 72 2020-03-19 08:08:03Z fujisft-shigihara $
#
...
=end
      ...
      hObjects.each{|sObjectID, cObjectInfo|
        ...
        unless (cObjectInfo.hState[TSR_PRM_VAR].nil?())
          ...
          # 値がセットされた変数のチェック
          if ((bIsPre == false) && cObjectInfo.is_value_set_variable?())
...
=end
      ...
      hObjects.each{|sObjectID, cObjectInfo|
        ...
        if (!cObjectInfo.hState[TSR_PRM_LOCMTX].nil?() && !cObjectInfo.hState[TSR_PRM_LOCMTX].empty?())
          cObjectInfo.hState[TSR_PRM_LOCMTX].each{|sMutexID|
            ### T4_046: 取得済みのミューテックスIDリストに登録されたミューテックスが存在しない
            if (!@hAllObject.has_key?(sMutexID))
              sErr = sprintf("T4_046: " + ERR_TARGET_NOT_DEFINED, sMutexID)
              aErrors.push(YamlError.new(sErr, aPath + [sObjectID, TSR_PRM_LOCMTX]))
            else
              cMutexInfo = get_object_info(sMutexID)
              # T4_047: 取得済みのミューテックスIDリストに登録されたミューテックスの，ミューテックス取得元のタスクが一致していない
              if (sObjectID != cMutexInfo.hState[TSR_PRM_HTSKID])
                sErr = sprintf("T4_047: " + ERR_LOCKED_MUTEX_TARGET_MISMATCH, sMutexID, sObjectID)
                aErrors.push(YamlError.new(sErr, aPath + [sObjectID, TSR_PRM_LOCMTX]))
              end
            end
          }
...
=end
      ...
      hObjects.each{|sObjectID, cObjectInfo|
              ...
              if (@hAllObject.has_key?(sTask) && @hAllObject[sTask].sObjectType == TSR_OBJ_TASK)
                hVar.each{|sVarName, aType|
                  ...
                  if (!hVarAtr.nil?() && hVarAtr.has_key?(sVarName))
                    ...
                    if (!aType.include?(hVarAtr[sVarName].sType))
                    ...
                    elsif (hVarAtr[sVarName].sType == TYP_UINT8_T)
                      # uint8_tの場合，配列型の変数であることをチェックする
                      # (uint8_tを使用するのはメッセージバッファのみとする)
                      if (hVarAtr[sVarName].nArraySize.nil?())
                        sErr = sprintf("T4_030: " + ERR_VARIABLE_TYPE_MISMATCH, sVarName, sTask, "array of #{TYP_UINT8_T}")
                        aErrors.push(YamlError.new(sErr, aPath + [sObjectID, TSR_PRM_RTSKLIST, sTask]))
                      end
                    end
        ...
        # メモリプール
        if (cObjectInfo.sObjectType == TSR_OBJ_MEMORYPOOL && !cObjectInfo.hState[TSR_PRM_KERMPF].nil?())
          hAllVariable.each{|sProcObjectID, hVar|
            hVar.each{|sVarName, cVar|
              if (sVarName == cObjectInfo.hState[TSR_PRM_KERMPF])
                ### T4_032: メモリプールの先頭番地を代入する変数を持つ処理単位が実行状態でない時にその変数を確認しようとしている
                unless (@hAllObject[sProcObjectID].is_activate?())
                  sErr = sprintf("T4_032: " + ERR_MPF_NOT_ACTIVATE, sProcObjectID, cObjectInfo.hState[TSR_PRM_KERMPF])
                  aErrors.push(YamlError.new(sErr, aPath + [sObjectID, TSR_PRM_KERMPF]))
                end
      ...
      # ミューテックス
      hObjects = get_objects_by_type(TSR_OBJ_MUTEX)
      hObjects.each{|sObjectID, cObjectInfo|
        if (!cObjectInfo.hState[TSR_PRM_HTSKID].nil?)
          ### T4_048: ミューテックス取得元のタスクに登録されたタスクが存在しない
          if (!@hAllObject.has_key?(cObjectInfo.hState[TSR_PRM_HTSKID]))
            sErr = sprintf("T4_048: " + ERR_TARGET_NOT_DEFINED, cObjectInfo.hState[TSR_PRM_HTSKID])
            aErrors.push(YamlError.new(sErr, aPath + [sObjectID, TSR_PRM_HTSKID]))
          else
            cTaskInfo = get_object_info(cObjectInfo.hState[TSR_PRM_HTSKID])
            # T4_047: 取得済みのミューテックスIDリストに登録されたミューテックスの，ミューテックス取得元のタスクが一致していない
            if (cObjectInfo.hState[TSR_PRM_MTXSTAT] == TSR_STT_TMTX_LOC)
              if (cTaskInfo.hState[TSR_PRM_LOCMTX].nil? || !cTaskInfo.hState[TSR_PRM_LOCMTX].include?(sObjectID))
                sErr = sprintf("T4_047: " + ERR_LOCKED_MUTEX_TARGET_MISMATCH, sObjectID, cObjectInfo.hState[TSR_PRM_HTSKID])
                aErrors.push(YamlError.new(sErr, aPath + [sObjectID, TSR_PRM_HTSKID]))
              end
            end
          end
    ...
    def is_time_control_situation?(hVariation)
      ...
      # CPU状態にsystimが指定されている
      hObjects = get_objects_by_type(TSR_OBJ_CPU_STATE)
      hObjects.each_value{|cObjectInfo|
        if (cObjectInfo.is_specified?(TSR_PRM_SYSTIM))
          bResult = true
        end
      }
