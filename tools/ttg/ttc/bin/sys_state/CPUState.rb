...
#  の責任を負わない．
#
#  $Id: CPUState.rb 47 2019-10-31 02:49:45Z fujisft-shigihara $
#
  ...
  class CPUState
      ...
      @hState.each{|sAtr, val|
        begin
          if (is_specified?(sAtr))
            ...
            # 0以上の整数か文字列
            when TSR_PRM_SYSTIM, TSR_PRM_ACCESS1, TSR_PRM_ACCESS2, TSR_PRM_ACCESS3, TSR_PRM_ACCESS4
              unless (val.is_a?(String))
    ...
    def object_check(bIsPre = false)
      ...
      if ((bIsPre == true) && @cConf.is_hrp?())
        ### T3_CPU002: access1～access4が揃って指定されていない(すべて省略も可)
        if (!((@hState[TSR_PRM_ACCESS1].nil?() && @hState[TSR_PRM_ACCESS2].nil?() &&
             ...
             (!@hState[TSR_PRM_ACCESS1].nil?() && !@hState[TSR_PRM_ACCESS2].nil?() &&
              !@hState[TSR_PRM_ACCESS3].nil?() && !@hState[TSR_PRM_ACCESS4].nil?())))
          aErrors.push(YamlError.new("T3_CPU002: " + ERR_NO_ALL_ACCESS_SET, @aPath))
        end
      end
    end
