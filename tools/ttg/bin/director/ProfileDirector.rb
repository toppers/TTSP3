...
#  の責任を負わない．
#
#  $Id: ProfileDirector.rb 70 2020-02-04 09:29:02Z fujisft-shigihara $
#
...
module TTG
  ...
  class ProfileDirector
    ...
    def build(aTestScenarios)
      ...
      # オーバランハンドラの情報を取得
      hOverrunInfo = get_all_object_info(aTestScenarios, TSR_OBJ_OVERRUN)
      # オーバランハンドラがあるときは共有処理単位/関数を定義する
      if (hOverrunInfo.empty?() == false)
        @cProfileBuilder.build_shared_proc_unit(hOverrunInfo, TSR_OBJ_OVERRUN)
      end

      # 拡張サービスコールの情報を取得
      hSVCInfo = get_all_object_info(aTestScenarios, TSR_OBJ_SVC)
      # 拡張サービスコールがあるときは共有処理単位/関数を定義する
      if (hSVCInfo.empty?() == false)
        @cProfileBuilder.build_shared_proc_unit(hSVCInfo, TSR_OBJ_SVC)
      end
      ...
      # CPU状態(SAC_SYS)を最後にまとめて登録する
      @cProfileBuilder.build_cpu_state_config()
    end

    #================================================================
    # 概  要: 指定したオブジェクトの全テストシナリオの情報を返す
    #         (割込み，CPU例外ハンドラ，オーバランハンドラ，周期ハンドラ(TA_STA)のみに使用する)
    #================================================================
    def get_all_object_info(aTestScenarios, sObjectType)
      ...
      aTestScenarios.each{|cTestScenario|
        if (((sObjectType == TSR_OBJ_INTHDR) && (cTestScenario.exist_int_hdr() == true)) ||
            ...
            ((sObjectType == TSR_OBJ_OVERRUN) && (cTestScenario.exist_overrun() == true)) ||
            ((sObjectType == TSR_OBJ_SVC) && (cTestScenario.exist_svc() == true)) ||
