...
#  の責任を負わない．
#
#  $Id: ASPBuilder.rb 60 2019-12-20 01:14:37Z fujisft-shigihara $
#
  ...
  class ASPBuilder < ProfileBuilder
    ...
    def build_pre_condition()
      ...
      if (@cTS.exist_pre_locked_mutex() == true)
        @cIMCode.add_element(@cTS.gc_pre_lock_mutex())
      end
      ...
      #
      # 起動時優先度とベース優先度が異なるタスクの設定(running,dormant以外のタスク)
      #
      ...
      if ((@cTS.exist_interrupt_req() == true) && (@cConf.get_api_support_ras_int() == true))
        @cIMCode.add_element(@cTS.gc_pre_interrupt_req())
      end
    ...
    def build_lastpost_condition(nIndex)
      ...
      if ((@cTS.exist_interrupt_req(nIndex) == true) && (@cConf.get_api_support_clr_int() == true))
        @cIMCode.add_element(@cTS.gc_lastpost_interrupt_clr())
      end
