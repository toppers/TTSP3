...
#  Copyright (C) 2019-2020 by FUJI SOFT INCORPORATED
#
...
module CommonModule
  ...
  class PreCondition < Condition
    ...
    def exist_pre_task_pri_chg_fmp()
      @hTask.each{|sObjectID, cObjectInfo|
        if ((cObjectInfo.hState[TSR_PRM_STATE] != KER_TTS_RUN) &&
          ...
          return true # [Bool]起動時優先度とベース優先度が異なるタスクがある場合
        end
      }

      return false # [Bool]起動時優先度とベース優先度が異なるタスクがない場合
    end
      ...
      aPrcid.each{|nPrcid|
        if (!@aRunning[nPrcid].nil?())
          ...
          if (@aRunning[nPrcid].sObjectType == TSR_OBJ_TASK)
            ...
            if (@aRunning[nPrcid].bPreLockMutex == false)
              cElement.set_syscall(hMainTaskInfo, "#{API_MACT_TSK}(#{@aRunning[nPrcid].sObjectID}, #{@aRunning[nPrcid].hState[TSR_PRM_PRCID]})")
            else
              cElement.set_syscall(hMainTaskInfo, "#{API_WUP_TSK}(#{@aRunning[nPrcid].sObjectID})")
            end
            # タスク保留例外要因があれば設定する
            ...
            if (@aRunning[nPrcid].hState[TSR_PRM_TSKBPRI] != @aRunning[nPrcid].hState[TSR_PRM_ITSKPRI])
              cElement.set_syscall(hMainTaskInfo, "#{API_CHG_PRI}(#{@aRunning[nPrcid].sObjectID}, #{@aRunning[nPrcid].hState[TSR_PRM_TSKPRI]})")
            end

            # タスク終了禁止状態設定
            if (@aRunning[nPrcid].hState[TSR_PRM_DISTER] == true)
              cElement.set_svc_syscall(get_proc_unit_info(@aRunning[nPrcid]), API_DIS_TER)
            end
            ...
            if (@aRunning[nPrcid].hState[TSR_PRM_PRCID] != @sMainPrcid)
              ...
              # タスク終了要求状態設定
              if (@aRunning[nPrcid].hState[TSR_PRM_RASTER] == true)
                # メインタスクと対象タスクのプロセッサが異なる場合はメインタスクを移動
                if (@sMainPrcid != nPrcid)
                  cElement.set_syscall(hMainTaskInfo, "#{API_MIG_TSK}(#{TTG_TSK_SELF}, #{nPrcid})")
                end
                cElement.set_syscall(hMainTaskInfo, "#{API_RAS_TER}(#{@aRunning[nPrcid].sObjectID})")
                if (@sMainPrcid != nPrcid)
              ...
              if (@aRunning[nPrcid].hState[TSR_PRM_RASTER] == true)
                if (@sMainPrcid != nPrcid)
                  ...
                  # メインプロセッサの場合，一時的に実行状態タスクへ切り替えてdis_terを発行した上でras_terを発行する
                  cElement.set_syscall(hMainTaskInfo, "#{API_SLP_TSK}()")
                  cElement.set_syscall(get_proc_unit_info(@aRunning[nPrcid]), "#{API_WUP_TSK}(#{TTG_MAIN_TASK})")
                end
          ...
          # タスク例外の場合
            ...
            if (@aRunning[nPrcid].bPreLockMutex == false)
              ...
              cElement.set_syscall(hMainTaskInfo, "#{API_WUP_TSK}(#{@aRunning[nPrcid].hState[TSR_PRM_TASK]})")
            end
    ...
    def gc_pre_task_running_suspended_fmp()
      ...
      @aTask.each{|nPrcid, sObjectID, cObjectInfo|
        if (cObjectInfo.hState[TSR_PRM_STATE] == KER_TTS_RUS)
          ...
          cElement.set_svc_syscall(hProcUnitInfo, FNC_SUS_TSK, sObjectID)
    ...
    def gc_pre_nontask_activate_fmp()
      ...
      aPrcid.each{|nPrcid|
        if (!@aActivate[nPrcid].nil?())
          ...
          when TSR_OBJ_ALARM
            # 0秒に設定して1ミリ秒進める
            cElement.set_svc_syscall(hProcUnitInfo, API_MSTA_ALM, @aActivate[nPrcid].sObjectID, 0, nPrcid)
          ...
          # 周期ハンドラ
          when TSR_OBJ_CYCLE
            cElement.set_svc_syscall(hProcUnitInfo, API_MSTA_CYC, @aActivate[nPrcid].sObjectID, nPrcid)
            # 位相の分だけ時間を進める
          ...
          when TSR_OBJ_INTHDR, TSR_OBJ_ISR
            ...
            # 割込み発生関数を実行して起動する
            if (@cConf.is_hrp?())
              cElement.set_code(hObjectInfo, "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_INT_RAISE]}, #{@aActivate[nPrcid].hState[TSR_PRM_INTNO]}, #{HRP_DUMMY_ARG4});")
            else
          ...
          when TSR_OBJ_EXCEPTION
            ...
            # CPU例外発生関数を実行して起動する
            if (@cConf.is_hrp?())
              cElement.set_code(hObjectInfo, "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_CPUEXC_RAISE]}, #{@aActivate[nPrcid].hState[TSR_PRM_EXCNO]}, #{HRP_DUMMY_ARG4});")
            else
      ...
      # タイムティックの供給
      nGainTick.times{
        gc_tick_gain_global_fmp(cElement)
      }
      ...
      if (!aWaitPrcid.empty?())
        aWaitPrcid.each{|nPrcid|
          ...
          # メインプロセッサで非タスクが動作している場合は，非タスク側でも待つ
          if (!@cActivate.nil?())
            hTempInfo = get_proc_unit_info(@cActivate)
            cElement.set_wait_check_sync(hTempInfo, nPrcid)
          end
        }
      end
        ...
        if (!cTaskInfo.hState[TSR_PRM_LOCMTX].nil?)
          cTaskInfo.hState[TSR_PRM_LOCMTX].each{|sMutexID|
            ...
            if (cTaskInfo.hState[TSR_PRM_PRCID] == @sMainPrcid)
              ...
              cElement.set_state_sync(get_proc_unit_info(), hTargetTaskInfo[:id], KER_TTS_WAI)
            end
      ...
      @aPrcid.each{ |nPrcid|
        ...
        if (!@aCpuState[nPrcid].nil?() &&
          ...
          if (@aActivate[nPrcid].nil?() || (@aActivate[nPrcid].sObjectType == TSR_OBJ_EXCEPTION))
            ...
            cElement.set_svc_syscall(hProcUnitInfo, API_CHG_IPM, @aCpuState[nPrcid].hState[TSR_PRM_CHGIPM])
    ...
    #=================================================================
    # 概  要: サブ優先度を設定するコードを返す処理を
    #         IMCodeElementクラスにまとめて返す
    #=================================================================
    def gc_pre_set_subpri()
      cElement = IMCodeElement.new()

      hProcUnitInfo = get_proc_unit_info(@cRunning)
      @hTask.each{|sObjectID, cObjectInfo|
        if (!cObjectInfo.hState[TSR_PRM_SUBPRI].nil?())
          cElement.set_syscall(hProcUnitInfo, "#{API_CHG_SPR}(#{cObjectInfo.sObjectID}, #{cObjectInfo.hState[TSR_PRM_SUBPRI]})")
        end
      }

      return cElement # [IMCodeElement]サブ優先度を設定するコード
    end
    ...
    def gc_pre_task_ready_porder_fmp()
      ...
      aPriOrderTemp.each_with_index{|aPriPrc, nPrcid|
        if (!aPriPrc.nil?())
          hProcUnitInfo = get_proc_unit_info(@aActivate[nPrcid], @aRunning[nPrcid])
        ...
        if (!cObjectInfo.hState[TSR_PRM_ACTCNT].nil?() && cObjectInfo.hState[TSR_PRM_ACTCNT] == 1)
          ...
          if (!cObjectInfo.hState[TSR_PRM_ACTPRC].nil?())
            if (!@cActivate.nil?())
              ...
              cElement.set_svc_syscall(hProcUnitInfo, API_MACT_TSK, sObjectID, cObjectInfo.hState[TSR_PRM_ACTPRC])
            end
          else
            if (!@cActivate.nil?())
      ...
      @hAllObject.each{|sObjectID, cObjectInfo|
        if ((cObjectInfo.sObjectType == TSR_OBJ_SPINLOCK) && (cObjectInfo.hState[TSR_PRM_SPNSTAT] == TSR_STT_TSPN_LOC))
          ...
          if (GRP_NON_CONTEXT.include?(cObject.sObjectType))
            ...
            cElement.set_svc_syscall(hProcUnitInfo, API_LOC_SPN, sObjectID)
          end
          cElement.set_checkpoint(hProcUnitInfo)
    ...
    #=================================================================
    def gc_pre_interrupt_req_fmp()
      cElement      = IMCodeElement.new()
      hMainTaskInfo = get_proc_unit_info()

      # ras_intする必要のある割込み番号とプロセッサIDを抽出
      aEnaIntNoID = []
      @hAllObject.each{|sObjectID, cObjectInfo|
        if ((GRP_INTERRUPT.include?(cObjectInfo.sObjectType) == true) &&
      ...
      aEnaIntNoID.each{|snIntNoID|
        ...
        cElement.set_syscall(hMainTaskInfo, "#{API_RAS_INT}(#{snIntNoID[0]})")
      }
