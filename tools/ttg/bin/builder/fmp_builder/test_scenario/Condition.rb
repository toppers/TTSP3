...
module CommonModule
  ...
  class Condition
    ...
    #=================================================================
    def check_activate_context_fmp(nPrcid)
    ...
    def gc_obj_ref_fmp()
      ...
      @hAllObject.each{|sObjectID, cObjectInfo|
        ...
        when TSR_OBJ_CPU_STATE, TSR_OBJ_INTHDR, TSR_OBJ_ISR
          ...
          if (!@aActivate[nPrcid].nil?())
            ...
            if (cObjectInfo.sObjectType == TSR_OBJ_CPU_STATE)
              cObjectInfo.gc_obj_ref(cElement, hTempInfo, check_activate_context_fmp(nPrcid))
            else
        ...
        when TSR_OBJ_TASK_EXC
          ...
          if (cTask.hState[TSR_PRM_STATE] != KER_TTS_DMT)
            cObjectInfo.gc_obj_ref(cElement, hProcUnitInfo, check_activate_context_fmp(nMainPrcid), bCpuLock)
          end
    ...
    def gc_tick_gain_fmp(cElement, aNextActivate, cNextRunning, nGainTick)
      ...
      if (!aPrcidProcUnitInfo.empty?())
        aPrcidProcUnitInfo.each{|aPrcidProcUnitInfo2|
          cElement.set_wait_check_sync(aPrcidProcUnitInfo2[0], aPrcidProcUnitInfo2[1])
        }
      end
    end
    ...
    #=================================================================
    # 概  要: システム時間を進める処理をcElementに格納する
    #=================================================================
    def gc_tick_gain_global_fmp(cElement)
      check_class(IMCodeElement, cElement)  # コードを追加するエレメント
