...
#  の責任を負わない．
#
#  $Id: FMPBuilder.rb 70 2020-02-04 09:29:02Z fujisft-shigihara $
#
...
module TTG
  ...
  class FMPBuilder < ProfileBuilder
    ...
    def build_pre_condition()
      ...
      #
      # 割込み要求がある割込みの設定(ras_intをサポートしている場合のみ)
      #
      if ((@cTS.exist_interrupt_req() == true) && (@cConf.get_api_support_ras_int() == true))
        @cIMCode.add_element(@cTS.gc_pre_interrupt_req_fmp())
      end
      ...
      #
      # ミューテックスをロックしているタスクの設定
      #
      if (@cTS.exist_pre_locked_mutex() == true)
        @cIMCode.add_element(@cTS.gc_pre_lock_mutex_fmp())
      end
      ...
      # サブ優先度の設定(HRMPは対象外)
      if (!@cConf.is_hrp?())
        @cIMCode.add_element(@cTS.gc_pre_set_subpri())
      end
      ...
      # システム時刻の設定
      #
      if (@cTS.check_set_tim() == true)
        @cIMCode.add_element(@cTS.gc_pre_set_tim())
      end
    ...
    def build_lastpost_condition(nIndex)
      ...
      #
      # 割込み要求がある割込みのクリア処理(clr_intをサポートしている場合のみ)
      #
      if ((@cTS.exist_interrupt_req(nIndex) == true) && (@cConf.get_api_support_clr_int() == true))
        @cIMCode.add_element(@cTS.gc_lastpost_interrupt_clr_fmp())
      end
    ...
    def build_initialize_test_lib_ini_rtn()
      ...
      # コンフィグファイル情報
      cElement.set_config("#{API_ATT_INI}({#{KER_TA_NULL}, 0, #{FNC_TEST_LIB_INI}});", IMC_NO_CLASS, TSR_STT_DOM_KERNEL)
