...
#  の責任を負わない．
#
#  $Id: ProfileBuilder.rb 72 2020-03-19 08:08:03Z fujisft-shigihara $
#
...
module TTG
  ...
  class ProfileBuilder
    ...
    def initialize()
      ...
      @bIsComplementedUserDomain = false  # ユーザドメインが補完されたか
    ...
    #================================================================
    # 概  要: CPU状態の静的APIを生成する
    #================================================================
    def build_cpu_state_config()
      cElement = IMCodeElement.new(:common)

      # ユーザドメインを補完した場合，アクセス権を付与する
      if (@cConf.is_hrp?() && (@bIsComplementedUserDomain == true))
        hMacro = @cConf.get_macro()
        sTaskDomain = hMacro["DOM_TASK"]
        cCpuState = CPUState.new("DUMMY_CPU_STATE", {TSR_PRM_ACCESS1 => "TACP(#{sTaskDomain})", TSR_PRM_ACCESS2 => "TACP(#{sTaskDomain})", TSR_PRM_ACCESS3 => "TACP(#{sTaskDomain})", TSR_PRM_ACCESS4 => "TACP(#{sTaskDomain})"}, ["DUMMY"], true)
      else
        cCpuState = CPUState.new("DUMMY_CPU_STATE", {}, ["DUMMY"], true)
      end

      cCpuState.gc_config(cElement)
    ...
    def build(cTestScenario, bPrevGainTime)
      ...
      # ユーザドメインが補完されたかを保持する
      if ((@bIsComplementedUserDomain == false) && (@cTS.exist_complemented_user_domain() == true))
        @bIsComplementedUserDomain = true
      end
    end
    ...
    def start_test_scenario(bPrevGainTime)
      ...
      # (割込みサービスルーチンの場合，1つのグローバル変数だけでよい)
      ...
      # ACTIVATEである拡張サービスコールがある場合，グローバル変数を定義，およびインクリメント
      if (@cTS.exist_svc() == true)
        aSvcID = @cTS.get_activate_proc_unit_id(TSR_OBJ_SVC)
        aSvcID.each{|sObjectID|
          cElement.set_global_var("#{sObjectID}_#{VAR_BOOTCNT}", "enum ENUM_#{sObjectID}", "#{sObjectID}_#{TTG_ENUM_INVALID}")
          cElement.set_code(hMainTask, "#{sObjectID}_#{VAR_BOOTCNT}++")
        }
      end
