...
#  の責任を負わない．
#
#  $Id: IntermediateCode.rb 72 2020-03-19 08:08:03Z fujisft-shigihara $
#
...
module TTG
  ...
  class IntermediateCode
    ...
    def initialize()
      ...
      @aDomainID = []        # ドメインID一覧
      @aTskpriForSubpri = [] # サブ優先度を使用する現在優先度
    ...
    attr_reader :aCode, :hProcUnitInfo, :aGlobalVar, :aHeader, :hConfig, :nStack, :aDomainID, :aTskpriForSubpri, :hMainTask, :hCheckMain
    ...
    def add_common_info()
      ...
      # 静的API情報の追加
      cElement.set_config("#{API_CRE_TSK}(#{TTG_MAIN_TASK}, {#{KER_TA_ACT}, 1, #{TTG_MAIN_TASK.downcase}, #{TTG_MAIN_PRI}, TTSP_TASK_STACK_SIZE, NULL});", @cConf.get_default_class(), TTG_MAIN_DOMAIN)

      # 後状態で全ドメインからメインタスクを起床，優先度変更できるようにアクセス権を付与する
      if (@cConf.is_hrp?())
        cElement.set_config("#{API_SAC_TSK}(#{TTG_MAIN_TASK}, {TACP_SHARED, TACP_SHARED, TACP_SHARED, TACP_SHARED});", @cConf.get_default_class(), TTG_MAIN_DOMAIN)
      end
      ...
      cElement.set_svc_syscall(@hCheckMain, FNC_REF_TSK, TTG_MAIN_TASK, "&#{GRP_VAR_TYPE[TYP_T_TTSP_RTSK]}")
    ...
    def add_element(cElement)
      ...
      cElement.aElement.each{|hElement|
        ...
        when :assert
          sCode = "#{FNC_CHECK_VALUE}(#{hElement[:var]}, #{hElement[:value]});"
        when :check
          sMainPrcid = @cConf.get_main_prcid()
          @aCheckSeqCnt[sMainPrcid] += 1
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_CHECK_POINT]}, #{@aCheckSeqCnt[sMainPrcid]}, #{HRP_DUMMY_ARG4});"
          else
        ...
        when :wait
          sMainPrcid = @cConf.get_main_prcid()
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_WAIT_CHECK_POINT]}, #{@aCheckSeqCnt[sMainPrcid]}, #{HRP_DUMMY_ARG4});"
          else
        ...
        when :check_mp
          ...
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_MP_CHECK_POINT]}, #{hElement[:proc_unit][:prcid]}, #{@aCheckSeqCnt[hElement[:proc_unit][:prcid]]}, #{HRP_DUMMY_ARG3});"
          else
            sCode = "#{FNC_MP_CHECK_POINT}(#{hElement[:proc_unit][:prcid]}, #{@aCheckSeqCnt[hElement[:proc_unit][:prcid]]});"
          end
        when :check_zero
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_CHECK_POINT]}, 0, #{HRP_DUMMY_ARG4});"
          else
        ...
        when :check_finish
          ...
          @aCheckSeqCnt[sMainPrcid] += 1
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_CHECK_FINISH]}, #{@aCheckSeqCnt[sMainPrcid]}, #{HRP_DUMMY_ARG4});"
          else
        ...
        when :check_finish_mp
          @aCheckSeqCnt[hElement[:proc_unit][:prcid]] += 1
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_MP_CHECK_FINISH]}, #{hElement[:proc_unit][:prcid]}, #{@aCheckSeqCnt[hElement[:proc_unit][:prcid]]}, #{HRP_DUMMY_ARG3});"
          else
            sCode = "#{FNC_MP_CHECK_FINISH}(#{hElement[:proc_unit][:prcid]}, #{@aCheckSeqCnt[hElement[:proc_unit][:prcid]]});"
          end
        when :waitcp
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_MP_WAIT_CHECK_POINT]}, #{hElement[:prcid]}, #{@aCheckSeqCnt[hElement[:prcid]]}, #{HRP_DUMMY_ARG3});"
          else
        ...
        when :barrier
          ...
          if (@cConf.is_hrp?())
            sCode = "(void) #{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_BARRIER_SYNC]}, #{@nBarrierSeqCnt}, #{hElement[:num]}, #{HRP_DUMMY_ARG3});"
          else
        ...
        when :postcode
          ...
          next
        when :domain
          @aDomainID.push(hElement[:id])
          @aDomainID.uniq!()
          next
        when :subpri
          @aTskpriForSubpri.push(hElement[:tskpri])
          @aTskpriForSubpri.uniq!()
          next
        else
    ...
    def add_header(hElement)
      ...
      when TSR_OBJ_OVERRUN
        aArgs = ["#{TYP_ID} #{@cConf.get_overrun_arg_name(1)}", "#{TYP_INTPTR_T} #{@cConf.get_overrun_arg_name(2)}"]
      when TSR_OBJ_SVC
        aArgs = ["#{TYP_INTPTR_T} #{@cConf.get_svc_arg_name(1)}", "#{TYP_INTPTR_T} #{@cConf.get_svc_arg_name(2)}",
                 "#{TYP_INTPTR_T} #{@cConf.get_svc_arg_name(3)}", "#{TYP_INTPTR_T} #{@cConf.get_svc_arg_name(4)}",
                 "#{TYP_INTPTR_T} #{@cConf.get_svc_arg_name(5)}", "#{TYP_ID} #{@cConf.get_svc_arg_name(6)}"]
    ...
    def add_config(hElement)
      ...
      if (@cConf.is_asp?())
        sClass = IMC_NO_CLASS
        sDomain = IMC_NO_DOMAIN

        # ASPではIMC_NO_CLASSをダミーで使用する
        ...
        sDomain = hElement[:domain]

        if (@hConfig[sClass].nil?())
          @hConfig[sClass] = {}
        end
        if (@hConfig[sClass][sDomain].nil?())
          @hConfig[sClass][sDomain] = []
        end

        @hConfig[sClass][sDomain].push(hElement[:code])
        @hConfig[sClass][sDomain].uniq!()
      elsif (@cConf.is_fmp?())
        sClass = hElement[:class]
        ...
        if (@hConfig[sDomain].nil?())
          @hConfig[sDomain] = []
        end

        @hConfig[sDomain].push(hElement[:code])
        @hConfig[sDomain].uniq!()
      end
    end
    private :add_config
    ...
    def add_chg_pri_main_task()
      ...
      cElement.set_local_var(hFuncInfo[:id], GRP_VAR_TYPE[TYP_T_TTSP_RTSK], TYP_T_TTSP_RTSK)
      cElement.set_svc_syscall(hFuncInfo, FNC_REF_TSK, TTG_MAIN_TASK, "&#{GRP_VAR_TYPE[TYP_T_TTSP_RTSK]}")
