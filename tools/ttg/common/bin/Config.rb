  ...
  class Config
    ...
    def set(sKey, val)
      ...
      when CFG_TEST_FLOW, CFG_DEBUG_MODE, CFG_STACK_SHARE, CFG_ALL_GAIN_TIME,
           ...
           CFG_SUPPORT_CLR_INT, CFG_SUPPORT_RAS_INT, CFG_SUPPORT_PRB_INT
        val = to_bool(val)
        ### T0_002: configureの設定値が指定された型と異なる
    ...
    def environment_check()
      ...
      if (hMacro.is_a?(Hash))
        ...
        hMacro.each{|sMacro, val|
          case sMacro
          # [0より大きい整数]
          when /^PRC_/, "MAIN_PRCID", "ANY_INI_BLKCNT", "ANY_NOW_BLKCNT", "ANY_BLKSZ", "ANY_MAX_SEMCNT", "ANY_NOW_SEMCNT", "ANY_MAX_MSIZE", "ANY_MBF_SIZE"
            if (!val.is_a?(Integer))
          ...
          # [文字列か0以上の整数]
          when /^INTNO_/, /^INHNO_/, /^EXCNO_/, /^ANY_/, "DOM_TASK", "DOM_NON_TASK"
            unless (val.is_a?(String))
              if (!val.is_a?(Integer))
                sErr = sprintf("T0_004: " + ERR_MCR_INVALID_TYPE, sMacro, "Integer or String", val.class())
                @aErrors.push(TTCError.new(sErr))
        ...
        # 依存関係のチェック
        check_priority("TSK_PRI_BOTTOM", "TSK_PRI_LOW", "TSK_PRI_MID", "TSK_PRI_HIGH", "TSK_PRI_TOP")
        check_le("TSK_PRI_LE_LE_4", "TSK_PRI_LE_4")
        check_le("TSK_PRI_LE_GE_13", "TSK_PRI_GE_13")
        check_priority("DATA_PRI_LOW", "DATA_PRI_MID", "DATA_PRI_HIGH", nil, nil, "DATA_PRI_MAX")
        check_priority("MSG_PRI_LOW", "MSG_PRI_MID", "MSG_PRI_HIGH", nil, nil, "MSG_PRI_MAX")
    ...
    # 概　要: 優先度の依存関係チェックを行う
    #=================================================================
    def check_priority(sBott, sLow, sMid, sHigh = nil, sTop = nil, sMax = nil)
      check_class(String, sBott)       # BOTTOM
      check_class(String, sLow)        # LOW
      check_class(String, sMid)        # MID
      check_class(String, sHigh, true) # HIGH
      check_class(String, sTop, true)  # TOP
    ...
    #=================================================================
    # 概　要: # オーバランハンドラの引数とする変数名を取得
    #=================================================================
    def get_overrun_arg_name(nNum)
      if (nNum == 1)
        return @@hConf[CFG_OVERRUN_ARG1_NAME] # [String]第1引数とする変数名
      elsif (nNum == 2)
        return @@hConf[CFG_OVERRUN_ARG2_NAME] # [String]第2引数とする変数名
      else
    ...
    #=================================================================
    # 概　要: # 拡張サービスコールの引数とする変数名を取得
    #=================================================================
    def get_svc_arg_name(nNum)
      case nNum
      when 1
        return @@hConf[CFG_SVC_ARG1_NAME] # [String]第1引数とする変数名
      when 2
        return @@hConf[CFG_SVC_ARG2_NAME] # [String]第2引数とする変数名
      when 3
        return @@hConf[CFG_SVC_ARG3_NAME] # [String]第3引数とする変数名
      when 4
        return @@hConf[CFG_SVC_ARG4_NAME] # [String]第4引数とする変数名
      when 5
        return @@hConf[CFG_SVC_ARG5_NAME] # [String]第5引数とする変数名
      when 6
        return @@hConf[CFG_SVC_ARG6_NAME] # [String]第6引数とする変数名
      else
        abort(ERR_MSG % [__FILE__, __LINE__])
      end
    end
    ...
    #=================================================================
    # 概　要: APIのclr_intをサポートしているかを返す
    #=================================================================
    def get_api_support_clr_int()
      return @@hConf[CFG_SUPPORT_CLR_INT] # [Bool]サポートしているか
    end

    #=================================================================
    # 概　要: APIのras_intをサポートしているかを返す
    #=================================================================
    def get_api_support_ras_int()
      return @@hConf[CFG_SUPPORT_RAS_INT] # [Bool]サポートしているか
    end

    #=================================================================
    # 概　要: APIのprb_intをサポートしているかを返す
    #=================================================================
    def get_api_support_prb_int()
      return @@hConf[CFG_SUPPORT_PRB_INT] # [Bool]サポートしているか
    end
    ...
    #=================================================================
    def is_fmp?()
      return ([CFG_PROFILE_FMP, CFG_PROFILE_HRMP].include?(@@hConf[CFG_PROFILE])) # [Bool]プロファイルがFMP or HRMPか
    end
    ...
    #=================================================================
    def is_hrp?()
      return ([CFG_PROFILE_HRP, CFG_PROFILE_HRMP].include?(@@hConf[CFG_PROFILE])) # [Bool]プロファイルがHRP or HRMPか
    end
