...
#  の責任を負わない．
#
#  $Id: Variable.rb 47 2019-10-31 02:49:45Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class Variable
    ...
    attr_accessor :sVarName, :sType, :snValue, :nArraySize, :hMember, :aPath
    ...
    def initialize(sVarName, hVarInfo, aPath)
      ...
      @sType      = nil         # 型
      @snValue    = nil         # 値
      @nArraySize = nil         # 配列の要素数
      @aArrayData = nil         # 配列の各要素データ
      @hMember    = nil         # 構造体のメンバ
    ...
    def store_var_info(hVarInfo)
      ...
      if (hVarInfo.is_a?(Hash))
        ...
        hVarInfo.each_key{|sKey|
          unless ((sKey == TSR_PRM_VAR_TYPE) || (sKey == TSR_PRM_VAR_ARRAY_SZ) || (sKey == TSR_PRM_VAR_VALUE))
            @hMember = {}
          end
        }
    ...
    def gc_global_local_var(cElement, sObjectID)
      ...
      if (GRP_GLOBAL_TYPE.include?(@sType))
        # 構造体でない場合
        if (@hMember.nil?())
          # グローバル変数指定用のデータ型(g_intptr_t)はintptr_tへ変更する
          if (@sType == TYP_GLOBAL_VAR)
            @sType = TYP_INTPTR_T
          end
      ...
      # ローカル変数
        ...
        # 配列の場合
        if (!@nArraySize.nil?())
          cElement.set_local_var(sObjectID, "#{@sVarName}[#{nArraySize}]", @sType, @snValue)
        # 構造体でない場合
        elsif (@hMember.nil?())
    ...
    def make_initial_str()
      ...
      STR_INITIAL_INFO[@sType].each{|aStructInfo|
        # カーネルによって，特定のメンバ変数を除外する
        if (@cConf.is_fmp?() && @cConf.is_hrp?())  # HRMP
          if (aStructInfo[0] == STR_SUBPRI)
            next
          end
        elsif (@cConf.is_asp?())
          if ([STR_PRCID, STR_ACTPRC, STR_SUBPRI, STR_SVCLEVEL].include?(aStructInfo[0]))
            next
          end
        elsif (@cConf.is_fmp?())
          if (aStructInfo[0] == STR_SVCLEVEL)
            next
          end
        elsif (@cConf.is_hrp?())
          if ([STR_PRCID, STR_ACTPRC, STR_SUBPRI,].include?(aStructInfo[0]))
            next
          end
        end
    ...
    def gc_assert_value(cElement, hProcUnitInfo)
      ...
      # 配列の値が設定されている場合
      if (!@aArrayData.nil?())
        @aArrayData.each_with_index{|sData, nIdx|
          cElement.set_assert(hProcUnitInfo, "#{@sVarName}[#{nIdx}]", sData)
        }
      # 変数の値が設定されている場合
      elsif (@hMember.nil?() && !@snValue.nil?())
