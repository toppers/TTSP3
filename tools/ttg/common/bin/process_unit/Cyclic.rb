...
#  の責任を負わない．
#
#  $Id: Cyclic.rb 60 2019-12-20 01:14:37Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class Cyclic < ProcessUnit
    ...
    def gc_config(cElement)
      ...
      sStaticApi = "#{API_CRE_CYC}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, {#{@hState[TSR_PRM_NFTTYPE]}"
      ...
      sStaticApi += "}, #{@hState[TSR_PRM_CYCTIM]}, #{@hState[TSR_PRM_CYCPHS]}});"

      cElement.set_config(sStaticApi, @hState[TSR_PRM_CLASS], @hState[TSR_PRM_DOMAIN]) # [IMCodeElement] 周期通知を生成する静的API

      # アクセス許可パターンを設定している場合
      if (@cConf.is_hrp?() && !@hState[TSR_PRM_ACCESS1].nil?())
        cElement.set_config("#{API_SAC_CYC}(#{@sObjectID}, {#{@hState[TSR_PRM_ACCESS1]}, #{@hState[TSR_PRM_ACCESS2]}, #{@hState[TSR_PRM_ACCESS3]}, #{@hState[TSR_PRM_ACCESS4]}});", @hState[TSR_PRM_CLASS], @hState[TSR_PRM_DOMAIN])
      end
    end
