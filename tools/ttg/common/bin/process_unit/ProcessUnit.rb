...
#  の責任を負わない．
#
#  $Id: ProcessUnit.rb 72 2020-03-19 08:08:03Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class ProcessUnit
    ...
    def initialize(sObjectID, hObjectInfo, sObjectType, aPath, bIsPre)
      ...
      # 処理単位クラス共通部分
      @hState[TSR_PRM_STATE]     = nil  # 状態[タスク/アラーム通知/周期通知/タスク例外/割込みハンドラ/割込みサービスルーチン/オーバランハンドラ]
      @hState[TSR_PRM_LEFTTMO]   = nil  # 時間指定[タスク/アラーム通知/周期通知]
      @hState[TSR_PRM_VAR]       = nil  # 変数情報[全処理単位]
      @hState[TSR_PRM_EXINF]     = nil  # 拡張情報[タスク/割込みサービスルーチン/初期化ルーチン/終了ルーチン]
      @hState[TSR_PRM_HDLSTAT]   = nil  # アラームハンドラ状態[アラームハンドラ/周期ハンドラ/タスク例外/割込みハンドラ/割込みサービスルーチン/CPU例外ハンドラ/拡張サービスコール]
      @hState[TSR_PRM_NFTTYPE]   = nil  # タイムイベントの通知の種類[アラーム通知/周期通知]
      @hState[TSR_PRM_NFYINFO1]  = nil  # タイムイベント通知用情報1[アラーム通知/周期通知]
      @hState[TSR_PRM_NFYINFO2]  = nil  # タイムイベント通知用情報2[アラーム通知/周期通知]
      @hState[TSR_PRM_ENFYTYPE]  = nil  # エラーの通知の種類[アラーム通知/周期通知]
      @hState[TSR_PRM_ENFYINFO1] = nil  # エラー通知用情報1[アラーム通知/周期通知]
      @hState[TSR_PRM_ENFYINFO2] = nil  # エラー通知用情報2[アラーム通知/周期通知]
      @hState[TSR_PRM_ACTPRC]    = nil  # 次回起動時割付けプロセッサID[タスク/アラームハンドラ/周期ハンドラ]
      @hState[TSR_PRM_BOOTCNT]   = nil  # 処理単位の起動回数[タスク/アラームハンドラ/周期ハンドラ/タスク例外/割込みハンドラ/割込みサービスルーチン/CPU例外ハンドラ/オーバランハンドラ/拡張サービスコール]
      ...
      @hState[TSR_PRM_ATR]       = nil  # 属性[周期ハンドラ/割込みハンドラ/割込みサービスルーチン/オーバランハンドラ]
      @hState[TSR_PRM_INTREQ]    = nil  # 割込み要求[割込みハンドラ/割込みサービスルーチン]
      @hState[TSR_PRM_DOMAIN]    = nil  # 所属ドメイン(タスク以外はKERNEL固定)
      ...
      @hState[TSR_PRM_TSKBPRI] = nil  # ベース優先度
      @hState[TSR_PRM_ITSKPRI] = nil  # 起動時優先度
      @hState[TSR_PRM_SUBPRI]  = nil  # サブ優先度
      ...
      @hState[TSR_PRM_RASTER]  = nil  # タスク終了要求状態
      @hState[TSR_PRM_DISTER]  = nil  # タスク終了禁止状態
      @hState[TSR_PRM_STKSZ]   = nil  # タスクのスタック領域のサイズ
      @hState[TSR_PRM_SSTKSZ]  = nil  # タスクのシステムスタック領域のサイズ
      @hState[TSR_PRM_STK]     = nil  # タスクのスタック領域の先頭番地
      @hState[TSR_PRM_SSTK]    = nil  # タスクのシステムスタック領域の先頭番地
      @hState[TSR_PRM_LOCMTX]  = nil  # 取得済みのミューテックスIDリスト
      ...
      # SVC
      @hState[TSR_PRM_FNCD]    = nil  # 拡張サービスコールの機能コード
      @hState[TSR_PRM_CALLER]  = nil  # 拡張サービスコールを呼出した処理単位
      @hState[TSR_PRM_ARGS]    = []   # 拡張サービスコールへの第1～5パラメータ
    ...
    def gc_obj_ref(cElement, hProcUnitInfo, bContext, bCpuLock)
      ...
      if (@sObjectType == TSR_OBJ_TASK_EXC)
        ...
        cElement.set_svc_syscall(hProcUnitInfo, @sRefAPI, @sObjectID, "&#{@sRefStrVar}")
      end
      ...
      # ベース優先度(HRP以外もしくはタスクが休止状態の場合は参照しない)
      if (!@hState[TSR_PRM_TSKBPRI].nil?() && (@hState[TSR_PRM_STATE] != KER_TTS_DMT))
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_TSKBPRI}", @hState[TSR_PRM_TSKBPRI])
      end
      ...
      # サブ優先度(FMPのみ/HRMPは対象外)
      if (!@hState[TSR_PRM_SUBPRI].nil? && @cConf.is_fmp?() && !@cConf.is_hrp?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_SUBPRI}", @hState[TSR_PRM_SUBPRI])
        if (@hState[TSR_PRM_TSKPRI].nil?())
          cElement.set_tskpri_for_subpri(@hState[TSR_PRM_ITSKPRI])
        else
          cElement.set_tskpri_for_subpri(@hState[TSR_PRM_TSKPRI])
        end
      end
      ...
      # タスク終了要求状態
      if (!@hState[TSR_PRM_RASTER].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_RASTER}", @hState[TSR_PRM_RASTER])
      end

      # タスク終了禁止状態
      if (!@hState[TSR_PRM_DISTER].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_DISTER}", @hState[TSR_PRM_DISTER])
      end
      ...
      # タイムイベント通知用情報1(データ構造上は拡張情報exinfとなる)
      if (!@hState[TSR_PRM_NFYINFO1].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{@sRefExInf}", "(#{TYP_INTPTR_T})#{@hState[TSR_PRM_NFYINFO1]}")
      end
      ...
      # ロック中のミューテックスリスト
      if (!@hState[TSR_PRM_LOCMTX].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_MTXCNT}", @hState[TSR_PRM_LOCMTX].size())
        if (!@hState[TSR_PRM_LOCMTX].empty?())
          cElement.set_local_var(hProcUnitInfo[:id], VAR_MTXID, TYP_ID)
          @hState[TSR_PRM_LOCMTX].reverse().each_with_index{|sMutexID, nIndex|
            cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_LOC_MTX, @sObjectID, nIndex + 1, "&#{VAR_MTXID}")
            cElement.set_assert(hProcUnitInfo, VAR_MTXID, sMutexID)
          }
        end
      end
      ...
      # 所属ドメイン
      # ・指定できるのはタスク/アラーム通知/周期通知のみ
      # ・ただし，HRPでは所属ドメイン情報を保持していないためチェック不可
      if (!@hState[TSR_PRM_DOMAIN].nil?() &&
          ((@cConf.is_fmp?() && [TSR_OBJ_TASK, TSR_OBJ_ALARM, TSR_OBJ_CYCLE].include?(@sObjectType)) ||
           (!@cConf.is_fmp?() && [TSR_OBJ_TASK].include?(@sObjectType))))
        if (@hState[TSR_PRM_DOMAIN] == TSR_STT_DOM_KERNEL)
          cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_DOMAIN}", KER_DOMAIN_KERNEL)
        else
          cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_DOMAIN}", @hState[TSR_PRM_DOMAIN])
        end
      end
      ...
      if (@cConf.is_hrp?())
        if (!@hState[TSR_PRM_ACCESS1].nil?())
          cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn1", @hState[TSR_PRM_ACCESS1])
          cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn2", @hState[TSR_PRM_ACCESS2])
          cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn3", @hState[TSR_PRM_ACCESS3])
          cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn4", @hState[TSR_PRM_ACCESS4])
        # アクセス許可パターン指定なし(所属ドメイン)
        else
          if (@hState[TSR_PRM_DOMAIN] == TSR_STT_DOM_KERNEL)
            (1..4).each{|nCnt|
              cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn#{nCnt}", KER_ACP_KERNEL)
            }
          else
            (1..4).each{|nCnt|
              cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn#{nCnt}", "TACP(#{@hState[TSR_PRM_DOMAIN]})")
            }
          end
        end
      end
    ...
    def gc_ini_rtn(bFirstFlg)
      ...
      # ASP/HRPで先頭の初期化ルーチンの場合，初期化処理を行う
      if ((bFirstFlg == true) && !@cConf.is_fmp?())
        # テストライブラリ用変数初期化
    ...
    def gc_finish_message()
      ...
      # GCOVの出力
      # FMPの場合はGCOV専用のグローバル終了ルーチンで出力するため，ASP/HRPのみ
      if (@cConf.enable_gcov?() && !@cConf.is_fmp?())
    ...
    def gc_ini_ter_rtn_info(cElement, hRtnInfo, bFirstFlg)
      ...
      # コンフィグファイル情報
      cElement.set_config("#{@sCfgAPI}({#{KER_TA_NULL}, #{@hState[TSR_PRM_EXINF]}, #{@sObjectID.downcase}});", @hState[TSR_PRM_CLASS], @hState[TSR_PRM_DOMAIN])
      ...
      if (@cConf.enable_gcov?())
        # ASP/HRPで最初の初期化ルーチンの場合，中断処理を入れる
        if (bFirstFlg == true && !@cConf.is_fmp?())
      ...
      if (@hState[TSR_PRM_DO].has_key?(TSR_PRM_SYSCALL))
        ...
        elsif (@hState[TSR_PRM_DO].has_key?(TSR_PRM_ERBOOL))
          cElement.set_syscall(hRtnInfo, @hState[TSR_PRM_DO][TSR_PRM_SYSCALL], @hState[TSR_PRM_DO][TSR_PRM_ERBOOL], TYP_ER_BOOL)
