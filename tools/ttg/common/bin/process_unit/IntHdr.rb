...
#  の責任を負わない．
#
#  $Id: IntHdr.rb 72 2020-03-19 08:08:03Z fujisft-shigihara $
#
...
module CommonModule
  ...
  class IntHdr < ProcessUnit
    ...
    #=================================================================
    # 概  要: 割込み要求を参照するコードをcElementに格納する
    #=================================================================
    def gc_obj_ref(cElement, hProcUnitInfo)
      ...
      # 割込み要求
      cElement.set_svc_syscall(hProcUnitInfo, API_PRB_INT, @hState[TSR_PRM_INTNO], nil, nil, nil, nil, @hState[TSR_PRM_INTREQ], TYP_ER_BOOL)
    end
    ...
    def gc_config(cElement)
      ...
      cElement.set_config("#{API_DEF_INH}(#{@hState[TSR_PRM_INHNO]}, {#{KER_TA_NULL}, #{@sObjectID.downcase}});", @hState[TSR_PRM_CLASS], @hState[TSR_PRM_DOMAIN])
    end
  end
end
