...
#  の責任を負わない．
#
#  $Id: Dataqueue.rb 70 2020-02-04 09:29:02Z fujisft-shigihara $
#
  ...
  class Dataqueue < SCObject
    ...
    def gc_config(cElement)
      ...
      if (!@hState[TSR_PRM_MB].nil?())
        sDtqmb = ", #{@hState[TSR_PRM_MB]}"
      end
      cElement.set_config("#{API_CRE_DTQ}(#{@sObjectID}, {#{@hState[TSR_PRM_ATR]}, #{@hState[TSR_PRM_DATACNT]}#{sDtqmb}});", @hState[TSR_PRM_CLASS], @hState[TSR_PRM_DOMAIN]) #[IMCodeElement] データキューを生成する静的API

      # アクセス許可パターンを設定している場合
      if (@cConf.is_hrp?() && !@hState[TSR_PRM_ACCESS1].nil?())
        cElement.set_config("#{API_SAC_DTQ}(#{@sObjectID}, {#{@hState[TSR_PRM_ACCESS1]}, #{@hState[TSR_PRM_ACCESS2]}, #{@hState[TSR_PRM_ACCESS3]}, #{@hState[TSR_PRM_ACCESS4]}});", @hState[TSR_PRM_CLASS], @hState[TSR_PRM_DOMAIN])
      end
    end
  end
end
