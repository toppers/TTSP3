...
module CommonModule
  ...
  class SCObject
    ...
    def initialize(sObjectID, hObjectInfo, sObjectType, aPath, bIsPre)
      ...
      @hState[TSR_PRM_WTSKLIST] = nil  # 待ちタスクのリスト[セマフォ/イベントフラグ/メールボックス/固定長メモリプール/ミューテックス]
      @hState[TSR_PRM_MSGLIST]  = nil  # 受信待ちメッセージのリスト[メールボックス/メッセージバッファ]
      @hState[TSR_PRM_ATR]      = nil  # 属性[セマフォ/イベントフラグ/データキュー/優先度データキュー/メールボックス/固定長メモリプール/ミューテックス/メッセージバッファ]
      @hState[TSR_PRM_MB]       = nil  # 管理領域の先頭番地[データキュー/優先度データキュー/固定長メモリプール]
      ...
      @hState[TSR_PRM_KERMPF]   = nil  # 固定長メモリプール領域の先頭番地(カーネルによって管理されている先頭番地)
      @hState[TSR_PRM_MPF]      = nil  # 固定長メモリプール領域の先頭番地(静的APIで指定する先頭番地)
      ...
      # MUTEX
      @hState[TSR_PRM_MTXSTAT]  = nil  # ミューテックスの状態
      @hState[TSR_PRM_CEILPRI]  = nil  # ミューテックスの上限優先度
      @hState[TSR_PRM_HTSKID]   = nil  # ミューテックスをロックしているタスクID

      # MESSAGEBUF
      @hState[TSR_PRM_MAXMSZ]  = nil  # 最大メッセージサイズ
      @hState[TSR_PRM_MBFSZ]   = nil  # 管理領域のサイズ
      @hState[TSR_PRM_FMBFSZ]  = nil  # 管理領域中の空き領域のサイズ
    ...
    def store_object_info(hObjectInfo)
      ...
      hObjectInfo.each{|atr, val|
        ...
        when TSR_PRM_DTQMB, TSR_PRM_PDQMB, TSR_PRM_MPFMB
          @hState[TSR_PRM_MB] = val
    ...
    def gc_obj_ref(cElement, hProcUnitInfo, bContext, bCouLock)
      ...
      # 共通部分
      # 待ちタスクのリスト[SEMAPHORE, EVENTFLAG, MAILBOX, MEMORYPOOL, MUTEX]
      if (!@hState[TSR_PRM_WTSKLIST].nil?())
        ...
        if (!@hState[TSR_PRM_WTSKLIST].empty?())
          ...
          @hState[TSR_PRM_WTSKLIST].each_with_index{|hTaskInfo, nIndex|
            case @sObjectType
            when TSR_OBJ_SEMAPHORE, TSR_OBJ_MAILBOX, TSR_OBJ_MEMORYPOOL, TSR_OBJ_MUTEX
              cElement.set_svc_syscall(hProcUnitInfo, @sRefRWaitAPI, @sObjectID, nIndex + 1, "&#{VAR_TSKID}")
            ...
            when TSR_OBJ_EVENTFLAG
              ...
              cElement.set_svc_syscall(hProcUnitInfo, @sRefRWaitAPI, @sObjectID, nIndex + 1, "&#{VAR_TSKID}", "&#{VAR_WAIPTN}", "&#{VAR_WFMODE}")
              cElement.set_assert(hProcUnitInfo, "#{VAR_TSKID}", hTaskInfo.keys[0])
          ...
          if (!@hState[TSR_PRM_DATALIST].empty?())
            ...
            when TSR_OBJ_DATAQUEUE
              ...
              @hState[TSR_PRM_DATALIST].each_with_index{|hData, nIndex|
                cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_DATA, @sObjectID, nIndex + 1, "&#{VAR_DATA}")
            ...
            when TSR_OBJ_P_DATAQUEUE
              ...
              @hState[TSR_PRM_DATALIST].each_with_index{|hData, nIndex|
                cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_PRI_DATA, @sObjectID, nIndex + 1, "&#{VAR_DATA}", "&#{VAR_DATAPRI}")
                cElement.set_assert(hProcUnitInfo, VAR_DATA, hData[TSR_VAR_DATA])
        ...
        if (!@hState[TSR_PRM_STSKLIST].empty?())
          ...
          when TSR_OBJ_DATAQUEUE
            ...
            @hState[TSR_PRM_STSKLIST].each_with_index{|hTaskInfo, nIndex|
              cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_SWAIT_DTQ, @sObjectID, nIndex + 1, "&#{VAR_TSKID}", "&#{VAR_DATA}")
              cElement.set_assert(hProcUnitInfo, VAR_TSKID, hTaskInfo.keys[0])
          ...
          when TSR_OBJ_P_DATAQUEUE
            ...
            cElement.set_local_var(hProcUnitInfo[:id], VAR_DATAPRI, TYP_PRI)
            @hState[TSR_PRM_STSKLIST].each_with_index{|hTaskInfo, nIndex|
              cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_SWAIT_PDQ, @sObjectID, nIndex + 1, "&#{VAR_TSKID}", "&#{VAR_DATA}", "&#{VAR_DATAPRI}")
              cElement.set_assert(hProcUnitInfo, VAR_TSKID, hTaskInfo.keys[0])
              cElement.set_assert(hProcUnitInfo, VAR_DATA, hTaskInfo[hTaskInfo.keys[0]][TSR_VAR_DATA])
          ...
          when TSR_OBJ_MESSAGEBUF
            cElement.set_local_var(hProcUnitInfo[:id], "#{VAR_MSG_DATA}[#{@hState[TSR_PRM_MAXMSZ]}]", TYP_UINT8_T)
            cElement.set_local_var(hProcUnitInfo[:id], VAR_MSG_SIZE, TYP_UINT_T)
            @hState[TSR_PRM_STSKLIST].each_with_index{|hTaskInfo, nIndex|
              cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_SWAIT_MBF, @sObjectID, nIndex + 1, "&#{VAR_TSKID}", "&#{VAR_MSG_DATA}[0]", "&#{VAR_MSG_SIZE}")
              cElement.set_assert(hProcUnitInfo, VAR_TSKID, hTaskInfo.keys[0])
              aTemp = hTaskInfo[hTaskInfo.keys[0]][TSR_PRM_MSG].gsub("{", "").gsub("}", "").split(",")
              aTemp.each_with_index{|sData, nDataIndex|
                cElement.set_assert(hProcUnitInfo, "#{VAR_MSG_DATA}[#{nDataIndex}]", sData)
              }
              cElement.set_assert(hProcUnitInfo, VAR_MSG_SIZE, hTaskInfo[hTaskInfo.keys[0]][TSR_PRM_MSGSZ])
            }
      ...
      if (!@hState[TSR_PRM_MSGLIST].nil?())
        ...
        if (!@hState[TSR_PRM_MSGLIST].empty?())
          ...
          when TSR_OBJ_MESSAGEBUF
            @hState[TSR_PRM_MSGLIST].each_with_index{|hData, nMsgIndex|
              cElement.set_local_var(hProcUnitInfo[:id], "#{VAR_MSG_DATA}[#{@hState[TSR_PRM_MAXMSZ]}]", TYP_UINT8_T)
              cElement.set_local_var(hProcUnitInfo[:id], VAR_MSG_SIZE, TYP_UINT_T)
              cElement.set_svc_syscall(hProcUnitInfo, FNC_REF_MBF_MSG, @sObjectID, nMsgIndex + 1, "&#{VAR_MSG_DATA}[0]", "&#{VAR_MSG_SIZE}")

              aTemp = hData[TSR_PRM_MSG].gsub("{", "").gsub("}", "").split(",")
              aTemp.each_with_index{|sData, nDataIndex|
                cElement.set_assert(hProcUnitInfo, "#{VAR_MSG_DATA}[#{nDataIndex}]", sData)
              }
              cElement.set_assert(hProcUnitInfo, VAR_MSG_SIZE, hData[TSR_PRM_MSGSZ])
            }
          else
            abort(ERR_MSG % [__FILE__, __LINE__])
          end
      ...
      if (!@hState[TSR_PRM_KERMPF].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_MPF}", @hState[TSR_PRM_KERMPF])
      end
      ...
      # MUTEX
      if (@hState[TSR_PRM_ATR] == KER_TA_CEILING)
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_CEILPRI}", @hState[TSR_PRM_CEILPRI])
      end
      if ((@hState[TSR_PRM_MTXSTAT] == TSR_STT_TMTX_LOC) && !@hState[TSR_PRM_HTSKID].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_HTSKID}", @hState[TSR_PRM_HTSKID])
      end

      # MESSAGEBUF
      if (!@hState[TSR_PRM_MAXMSZ].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_MAXMSZ}", @hState[TSR_PRM_MAXMSZ])
      end
      if (!@hState[TSR_PRM_MBFSZ].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_MBFSZ}", @hState[TSR_PRM_MBFSZ])
      end
      if (!@hState[TSR_PRM_FMBFSZ].nil?())
        cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_FMBFSZ}", @hState[TSR_PRM_FMBFSZ])
      end
      ...
      if (@cConf.is_hrp?())
        ...
        # アクセス許可パターン指定なし(所属ドメイン)
          ...
          elsif (@hState[TSR_PRM_DOMAIN] == TSR_STT_DOM_NONE)
            (1..4).each{|nCnt|
              cElement.set_assert(hProcUnitInfo, "#{@sRefStrVar}.#{STR_ACVCT}.acptn#{nCnt}", KER_ACP_SHARED)
            }
          else
            (1..4).each{|nCnt|
    ...
    #=================================================================
    def get_cpu_lock(bContext)
      check_class(Bool, bContext)
      if (bContext == true)
        if (@cConf.is_hrp?())
          sCpuLock = "#{API_CAL_SVC}(#{HRP_FN_TABLE[API_LOC_CPU]}, #{HRP_DUMMY_ARG5})"
        else
    ...
    #=================================================================
    def get_cpu_unlock(bContext)
      check_class(Bool, bContext)

      if (bContext == true)
        if (@cConf.is_hrp?())
          sCpuUnlock = "#{API_CAL_SVC}(#{HRP_FN_TABLE[API_UNL_CPU]}, #{HRP_DUMMY_ARG5})"
        else
