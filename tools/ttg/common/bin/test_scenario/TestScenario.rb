...
module CommonModule
  ...
  class TestScenario
    ...
    #=================================================================
    # 概  要: システム時刻指定の確認(システム時刻が指定されている場合はtrue)
    #=================================================================
    def check_set_tim(nIndex = nil)
      check_class(Integer, nIndex, true)  # 何番目のdo_postか  # 何番目のdo_postか

      if (nIndex.nil?())
        return @cPreCondition.check_set_tim()  # [Bool]システム時刻が指定されているか
      else
        return @aDo_PostCondition[nIndex].check_set_tim()  # [Bool]システム時刻が指定されているか
      end
    end
    ...
    #=================================================================
    # 概  要: 起動時優先度とベース優先度が異なるタスクの有無確認
    #=================================================================
    def exist_pre_task_pri_chg()
      return @cPreCondition.exist_pre_task_pri_chg()  # [Bool]起動時優先度とベース優先度が異なるタスクが存在するか
    end
    ...
    #=================================================================
    # 概  要: 割込み要求がある状態の割込みハンドラ，割込みサービスルー
    #         チンの有無確認
    #=================================================================
    def exist_interrupt_req(nIndex = nil)
      check_class(Integer, nIndex, true)  # 何番目のdo_postか

      if (nIndex.nil?())
        return @cPreCondition.exist_interrupt_req()  # [Bool]割込み要求がある割込みハンドラ，割込みサービスルーチンが存在するか
      else
        return @aDo_PostCondition[TTG_IDX_LAST].exist_interrupt_req()  # [Bool]割込み要求がある割込みハンドラ，割込みサービスルーチンが存在するか
      end
    end
    ...
    def exist_pre_locked_mutex()
      return @cPreCondition.exist_pre_locked_mutex()  # [Bool]ロック中のミューテックスの有無
    end
    ...
    def exist_complemented_user_domain()
      return @cPreCondition.exist_complemented_user_domain()  # [Bool]ユーザドメインが補完されたか
    end
    ...
    def gc_pre_interrupt_req()
      return @cPreCondition.gc_pre_interrupt_req()  # [IMCodeElement]割込み要求がある割込みを設定するコード
    end
    ...
    def gc_pre_task_pri_chg()
      return @cPreCondition.gc_pre_task_pri_chg()  # [IMCodeElement]起動時優先度とベース優先度が異なるタスクを設定するコード
    end
    ...
    def gc_pre_lock_mutex()
      return @cPreCondition.gc_pre_lock_mutex()  # [IMCodeElement]ミューテックスをロックするコード
    end
    ...
    #=================================================================
    # 概  要: システム時刻の設定（実行中の処理単位から設定）
    #=================================================================
    def gc_pre_set_tim()
      return @cPreCondition.gc_pre_set_tim()  # [IMCodeElement]システム時刻の設定をするコード
    end
    ...
    #=================================================================
    # 概  要: 割込み要求がある割込みをクリアするコードを返す
    #=================================================================
    def gc_lastpost_interrupt_clr()
      return @aDo_PostCondition[TTG_IDX_LAST].gc_lastpost_interrupt_clr()  # [IMCodeElement]割込み要求がある割込みをクリアするコード
    end
    ...
    #=================================================================
    def exist_overrun()
      return @cPreCondition.bIsOverrun  # [Bool]オーバランハンドラが存在するか
    end
    ...
    #=================================================================
    def exist_svc()
      return @cPreCondition.bIsSVC  # [Bool]拡張サービスコールが存在するか
    end
