...
#  の責任を負わない．
#
#  $Id: Condition.rb 60 2019-12-20 01:14:37Z fujisft-shigihara $
#
...
require "common/bin/process_unit/Overrun.rb"
require "common/bin/process_unit/SVC.rb"
...
require "common/bin/sc_object/Mutex.rb"
require "common/bin/sc_object/MessageBuf.rb"
...
module CommonModule
  ...
  class Condition
    ...
    attr_reader :bIsIniRtn, :bIsTerRtn, :bIsIntHdr, :bIsISR, :bIsException, :bIsOverrun, :bIsSVC, :lMainTaskState
    ...
    def initialize(sTestID)
      ...
      @bIsOverrun   = false    # オーバランハンドラの有無
      @bIsSVC       = false    # 拡張サービスコールの有無
      ...
      hCondition.each{|sObjectID, hObjectInfo|
        ...
        unless (hObjectType.has_key?(sObjectID))
          ...
          begin
            ...
            when TSR_OBJ_MUTEX
              cNewObject = Mutex.new(sObjectID, hObjectInfo, aPath, bIsPre)
            when TSR_OBJ_MESSAGEBUF
              cNewObject = MessageBuf.new(sObjectID, hObjectInfo, aPath, bIsPre)
            when TSR_OBJ_INTHDR
            ...
            when TSR_OBJ_OVERRUN
              cNewObject = Overrun.new(sObjectID, hObjectInfo, aPath, bIsPre)
              @bIsOverrun = true
            when TSR_OBJ_SVC
              cNewObject = SVC.new(sObjectID, hObjectInfo, aPath, bIsPre)
              @bIsSVC = true
            when TSR_OBJ_DOMAIN
              cNewObject = Domain.new(sObjectID, hObjectInfo, aPath, bIsPre)
            when TSR_OBJ_MEMORY_REGION
              cNewObject = MemoryRegion.new(sObjectID, hObjectInfo, aPath, bIsPre)
            when TSR_OBJ_MEMORY_SECTION
              cNewObject = MemorySection.new(sObjectID, hObjectInfo, aPath, bIsPre)
            when TSR_OBJ_MEMORY_MODULE
              cNewObject = MemoryModule.new(sObjectID, hObjectInfo, aPath, bIsPre)
    ...
    def check_activate_context()
      if (@cActivate.nil?())
        return true # [Bool]非タスクがない場合
      else
        return false # [Bool]非タスクがある場合
      end
    end
    ...
    #=================================================================
    # 概  要: システム時刻が指定されている場合はtrueを，そうでない場合はfalseを返す
    #=================================================================
    def check_set_tim()
      unless (@cCpuState.nil?() || @cCpuState.hState[TSR_PRM_SYSTIM].nil?())
        return true # [Bool] システム時刻が指定されている場合
      end

      return false # [Bool] システム時刻が指定されていない場合
    end
    ...
    #=================================================================
    # 概  要: 割込み要求がある割込みハンドラ，割込みサービスルーチンがいる
    #         場合はtrueを，そうでない場合はfalseを返す
    #=================================================================
    def exist_interrupt_req()
      @hAllObject.each{|sObjectID, cObjectInfo|
        if ((GRP_INTERRUPT.include?(cObjectInfo.sObjectType) == true) && (cObjectInfo.hState[TSR_PRM_INTREQ] == true))
          return true # [Bool] 許可状態の割込みハンドラ，割込みサービスルーチンがいる場合
        end
      }
    ...
    def gc_obj_ref()
      ...
      # 拡張サービスコールがACTIAVTEだった場合，各パラメータが引数と一致していることを確認するコードを入れる
      if (!@cActivate.nil?() && @cActivate.sObjectType == TSR_OBJ_SVC)
        @cActivate.gc_assert_args(cElement, hProcUnitInfo)
      end
      ...
      @hAllObject.each{|sObjectID, cObjectInfo|
        ...
        when TSR_OBJ_CPU_STATE
          cObjectInfo.gc_obj_ref(cElement, hProcUnitInfo, check_activate_context())
