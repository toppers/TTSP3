...
#  の責任を負わない．
#
#  $Id: IMCodeElement.rb 72 2020-03-19 08:08:03Z fujisft-shigihara $
#
  ...
  class IMCodeElement
    ...
    def set_syscall(hProcUnitInfo, sSyscall, snbReturn = "E_OK", sVarType = TYP_ER)
      ...
      if (!snbReturn.nil?())
        ...
        # 戻り値の型がboolean系以外の場合はcheck_ercd
        if (![TYP_BOOL_T, TYP_ER_BOOL].include?(sVarType))
          @aElement.push({:atr => :ercd, :proc_unit => hProcUnitInfo, :ret => snbReturn, :var => sVarName})
        # 戻り値の型がTYP_BOOL_T，TYP_ER_BOOLの場合はassert
        else
    ...
    #================================================================
    # 概　要: HRPではSVC経由としたAPI呼び出しと戻り値のチェックを行う
    #         コードの要素を保持する
    #================================================================
    def set_svc_syscall(hProcUnitInfo, sSyscall, snbArg1 = nil, snbArg2 = nil, snbArg3 = nil, snbArg4 = nil, snbArg5 = nil, snbReturn = "E_OK", sVarType = TYP_ER)
      check_class(Hash, hProcUnitInfo)                       # コードを追加する処理単位情報
      check_class(String, sSyscall)                          # 追加するAPI
      check_class([String, Integer, *Bool], snbArg1, true)   # 引数1（デフォルト nil）
      check_class([String, Integer, *Bool], snbArg2, true)   # 引数2（デフォルト nil）
      check_class([String, Integer, *Bool], snbArg3, true)   # 引数3（デフォルト nil）
      check_class([String, Integer, *Bool], snbArg4, true)   # 引数4（デフォルト nil）
      check_class([String, Integer, *Bool], snbArg5, true)   # 引数5（デフォルト nil）
      check_class([String, Integer, *Bool], snbReturn, true) # 想定される戻り値（デフォルト E_OK）
      check_class(String, sVarType)                          # 想定される戻り値の型（デフォルト TYP_ER）

      sArgStr = ""
      sSvcArgStr = ""
      sSvcDummyArg = HRP_DUMMY_ARG5
      if (!snbArg1.nil?)
        sArgStr = snbArg1
        sSvcArgStr = "(#{TYP_INTPTR_T})(#{snbArg1})"
        sSvcDummyArg = ", #{HRP_DUMMY_ARG4}"
      end
      if (!snbArg2.nil?)
        sArgStr += ", #{snbArg2}"
        sSvcArgStr += ", (#{TYP_INTPTR_T})(#{snbArg2})"
        sSvcDummyArg = ", #{HRP_DUMMY_ARG3}"
      end
      if (!snbArg3.nil?)
        sArgStr += ", #{snbArg3}"
        sSvcArgStr += ", (#{TYP_INTPTR_T})(#{snbArg3})"
        sSvcDummyArg = ", #{HRP_DUMMY_ARG2}"
      end
      if (!snbArg4.nil?)
        sArgStr += ", #{snbArg4}"
        sSvcArgStr += ", (#{TYP_INTPTR_T})(#{snbArg4})"
        sSvcDummyArg = ", #{HRP_DUMMY_ARG1}"
      end
      if (!snbArg5.nil?)
        sArgStr += ", #{snbArg5}"
        sSvcArgStr += ", (#{TYP_INTPTR_T})(#{snbArg5})"
        sSvcDummyArg = ""
      end

      if (@cConf.is_hrp?())
        set_syscall(hProcUnitInfo, "#{API_CAL_SVC}(#{HRP_FN_TABLE[sSyscall]}, #{sSvcArgStr}#{sSvcDummyArg})", snbReturn, TYP_ER_UINT)
      else
        set_syscall(hProcUnitInfo, "#{sSyscall}(#{sArgStr})", snbReturn, sVarType)
      end
    end
    ...
    def set_checkpoint(hProcUnitInfo)
      ...
      elsif (@cConf.is_asp?() || @cConf.is_hrp?())
    ...
    def set_state_sync(hProcUnitInfo, sTargetTask, sTargetState)
      ...
      if (@cConf.is_hrp?())
        set_code(hProcUnitInfo, "#{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_STATE_SYNC]}, (#{TYP_INTPTR_T})\"#{hProcUnitInfo[:id]}\", (#{TYP_INTPTR_T})\"#{sTargetTask}\", #{sTargetTask}, (#{TYP_INTPTR_T})\"#{sTargetState}\", #{sTargetState})")
      else
    ...
    def set_wait_finish_sync(hProcUnitInfo)
      ...
      if (@cConf.is_hrp?())
        set_code(hProcUnitInfo, "#{API_CAL_SVC}(#{HRP_FN_TABLE[FNC_WAIT_FINISH_SYNC]}, (#{TYP_INTPTR_T})\"#{hProcUnitInfo[:id]}\", #{HRP_DUMMY_ARG4})")
      else
    ...
    # 概　要: 静的APIの要素を保持する
    #================================================================
    def set_config(sCode, sClass = nil, sDomain = nil)
      check_class(String, sCode)         # 追加する静的API
      check_class(String, sClass, true)  # 追加する対象クラス(デフォルト nil)
      check_class(String, sDomain, true) # 追加する対象ドメイン(デフォルト nil)

      @aElement.push({:atr => :config, :code => sCode, :class => sClass, :domain => sDomain})
    end
    ...
    # 概　要: ドメインIDを保持する
    #================================================================
    def set_domain(sDomainID)
      check_class(String, sDomainID)  # ドメインID

      @aElement.push({:atr => :domain, :id => sDomainID})
    end


    #================================================================
    # 概　要: サブ優先度を使用する現在優先度を保持する
    #================================================================
    def set_tskpri_for_subpri(nTskpri)
      check_class(Integer, nTskpri)  # 現在優先度

      @aElement.push({:atr => :subpri, :tskpri => nTskpri})
    end
